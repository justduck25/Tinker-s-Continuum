...
public class WorldClientEvents extends ClientEventBase {
  ...
  static void addResourceListener(AddClientReloadListenersEvent event) {
    for (FoliageType type : FoliageType.values()) {
      event.addListener(TConstruct.getResource("slime_colors/" + type.getSerializedName()), new SlimeColorReloadListener(type));
    }
  }
  ...
  static void registerSkullModels(EntityRenderersEvent.CreateSkullModels event) {
    SkullModelHelper.HEAD_LAYERS.forEach((type, layer) -> {
      if (type.isPiglin()) {
        event.registerSkullModel(type, layer, PiglinHeadModel::new, SkullBlockRenderer.SKIN_BY_TYPE.get(type));
      } else {
        event.registerSkullModel(type, layer, SkullModel::new, SkullBlockRenderer.SKIN_BY_TYPE.get(type));
      }
    });
  }
  ...
  static void registerBlockColorHandlers(RegisterColorHandlersEvent.BlockTintSources event) {
    for (FoliageType type : FoliageType.values()) {
      event.register(List.of(slimeColor(type, null)),
      ...
      event.register(List.of(slimeColor(type, SlimeColorizer.LOOP_OFFSET)), TinkerWorld.slimeLeaves.get(type));
      event.register(List.of(slimeColor(type, null)), TinkerWorld.slimeFern.get(type), TinkerWorld.slimeTallGrass.get(type), TinkerWorld.pottedSlimeFern.get(type));
    }

    event.register(List.of(slimeColor(FoliageType.SKY, SlimeColorizer.LOOP_OFFSET)), TinkerWorld.skySlimeVine.get());
    event.register(List.of(slimeColor(FoliageType.ENDER, SlimeColorizer.LOOP_OFFSET)), TinkerWorld.enderSlimeVine.get());
  }

  /** Creates a position-aware slime foliage tint source. */
  private static BlockTintSource slimeColor(FoliageType type, @Nullable BlockPos add) {
    return new BlockTintSource() {
      ...
      public int colorInWorld(BlockState state, BlockAndTintGetter level, BlockPos pos) {
        return 0xFF000000 | getSlimeColorByPos(pos, type, add);
      }
