...
public final class TinkerStructures extends TinkerModule {
  ...
  public TinkerStructures() {
    FEATURES.register(TConstruct.MOD_EVENT_BUS);
    STRUCTURE_TYPE.register(TConstruct.MOD_EVENT_BUS);
    STRUCTURE_PIECE.register(TConstruct.MOD_EVENT_BUS);
    TREE_DECORATORS.register(TConstruct.MOD_EVENT_BUS);
    ROOT_PLACERS.register(TConstruct.MOD_EVENT_BUS);
  }
   ...
   * Misc
   */
  public static final DeferredHolder<TreeDecoratorType<?>, TreeDecoratorType<LeaveVineDecorator>> leaveVineDecorator = TREE_DECORATORS.register("leave_vines", () -> new TreeDecoratorType<LeaveVineDecorator>(LeaveVineDecorator.CODEC.fieldOf("config")));
  public static final DeferredHolder<RootPlacerType<?>, RootPlacerType<ExtraRootVariantPlacer>> extraRootVariantPlacer = ROOT_PLACERS.register("extra_root_variants", () -> new RootPlacerType<ExtraRootVariantPlacer>(ExtraRootVariantPlacer.CODEC.fieldOf("config")));
  ...
  @SubscribeEvent
    ...
    generator.addProvider(true, new StructureRepalleter(packOutput, event.getResourceManager(PackType.SERVER_DATA)));

//    generator.addProvider(server, new StructureUpdater(packOutput, event.getResourceManager(PackType.DATA), TConstruct.MOD_ID, Target.DATA_PACK, "structures"));
//    generator.addProvider(event.includeClient(), new StructureUpdater(packOutput, event.getResourceManager(PackType.CLIENT_RESOURCES), TConstruct.MOD_ID, Target.RESOURCE_PACK, "book/structures"));
  }
}
