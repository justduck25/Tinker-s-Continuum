...
public class MeltingModule implements IMeltingContainer, ContainerData {
  ...
  private IMeltingRecipe findRecipe() {
    ...
    IMeltingRecipe newRecipe = world.getServer().getRecipeManager().getRecipes().stream()
      .map(holder -> holder.value())
      .filter(recipe -> recipe instanceof IMeltingRecipe)
      .map(recipe -> (IMeltingRecipe) recipe)
  ...
  public CompoundTag writeToTag() {
    ...
    if (!stack.isEmpty()) {
      nbt.merge(ItemStackNbtHelper.save(stack));
  ...
  public void readFromTag(CompoundTag nbt) {
    stack = ItemStackNbtHelper.parse(nbt);
    if (!stack.isEmpty()) {
      currentTime = nbt.getInt(TAG_CURRENT_TIME).orElse(0);
      requiredTime = nbt.getInt(TAG_REQUIRED_TIME).orElse(0);
      requiredTemp = nbt.getInt(TAG_REQUIRED_TEMP).orElse(0);
    }
  }
