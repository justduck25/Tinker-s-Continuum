...
public class SolidFuelModule extends FuelModule {
  ...
  private void fetchHandlers() {
    ...
    IFluidHandler fluid = FluidTransferHelper.getFluidHandler(level, fuelPos, null);
    fluidHandler = fluid != null ? fluid : EmptyFluidHandler.INSTANCE;

    BlockEntity blockEntity = level.getBlockEntity(fuelPos);
    itemHandler = blockEntity instanceof HeaterBlockEntity heater ? heater.getItemCapability() : EmptyItemHandler.INSTANCE;
  }
  ...
  public int findFuel(boolean consume) {
    ...
    if (fluidHandler != null) {
      FluidStack fuelFluid = fluidHandler.getFluidInTank(0);
      if (!fuelFluid.isEmpty()) {
        temperature = tryLiquidFuel(fluidHandler, consume);
      }
      if (consume || temperature > 0) {
      }
    }
    // next, try solid fuel
