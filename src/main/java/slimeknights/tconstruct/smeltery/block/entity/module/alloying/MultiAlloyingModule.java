  ...
  private List<AlloyRecipe> getRecipes() {
    if (lastRecipes == null) {
      ...
      if (level.getServer() == null) {
        lastRecipes = Collections.emptyList();
      } else {
        lastRecipes = new ArrayList<>(level.getServer().getRecipeManager().getRecipes().stream()
          .map(holder -> holder.value())
          .filter(recipe -> recipe instanceof AlloyRecipe)
          .map(recipe -> (AlloyRecipe) recipe)
