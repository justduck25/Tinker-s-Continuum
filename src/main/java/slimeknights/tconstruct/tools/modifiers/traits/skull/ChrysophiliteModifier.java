...
public class ChrysophiliteModifier extends NoLevelsModifier implements EquipmentChangeModifierHook {
  ...
  public static int getTotalGold(@Nullable Entity entity) {
    if (entity instanceof LivingEntity living) {
      ...
      return gold == null ? 0 : gold.getTotalGold();
    }
    return 0;
  }

  /** Checks if the given stack has the vanishing curse. */
  private static boolean hasVanishingCurse(LivingEntity target, ItemStack stack) {
    ...
    if (source != null) {
      ...
      if (gold > 0) {
        ...
        for (EquipmentSlot slot : EquipmentSlot.values()) {
          ...
          if (!stack.isEmpty() && !hasVanishingCurse(target, stack) && stack.makesPiglinsNeutral(target) && random.nextFloat() < extraChance) {
            ...
            if (target.level() instanceof ServerLevel serverLevel) {
              var drop = target.spawnAtLocation(serverLevel, stack);
              if (drop != null) {
