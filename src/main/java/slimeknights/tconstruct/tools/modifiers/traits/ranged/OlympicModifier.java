...
public class OlympicModifier extends Modifier implements ProjectileLaunchModifierHook, ProjectileHitModifierHook {
  ...
  public boolean onProjectileHitEntity(ModifierNBT modifiers, ModDataNBT persistentData, ModifierEntry modifier, Projectile projectile, EntityHitResult hit, @Nullable LivingEntity attacker, @Nullable LivingEntity target, boolean notBlocked) {
    ...
    if (notBlocked && level instanceof ServerLevel serverLevel && targetEntity.getType().getCategory() == MobCategory.MONSTER && RANDOM.nextInt(20) < modifier.getLevel()) {
      ...
      if (!startCompound.isEmpty() && startCompound.contains("X") && startCompound.contains("Y") && startCompound.contains("Z")) {
        ...
        Item nugget = getNugget(targetEntity.distanceToSqr(startCompound.getDoubleOr("X", targetEntity.getX()), startCompound.getDoubleOr("Y", targetEntity.getY()), startCompound.getDoubleOr("Z", targetEntity.getZ())));
        if (nugget != Items.AIR) {
