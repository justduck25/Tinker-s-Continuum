...
public class RepairKitItem extends MaterialItem implements IRepairKitItem {
  ...
  public void appendHoverText(ItemStack stack, Item.TooltipContext context, TooltipDisplay display, Consumer<Component> tooltip, TooltipFlag flag) {
    ...
    if (canRepairInCraftingTable()) {
      tooltip.accept(Component.translatable(TOOLTIP_KEY, TranslationHelper.COMMA_FORMAT.format(getRepairAmount())).withStyle(ChatFormatting.GRAY));
    }
  }
  ...
  public boolean overrideStackedOnOther(ItemStack stack, Slot slot, ClickAction action, Player player) {
    ...
    if (canRepairInCraftingTable() && action == ClickAction.SECONDARY && slot.allowModification(player)) {
      ...
      if (!toolItem.isEmpty() && toolItem.is(TinkerTags.Items.MODIFIABLE)) {
        ...
        MaterialId material = getMaterial(stack).getMaterialId();
        ...
        if (tool.getDamage() > 0 && material != IMaterial.UNKNOWN_ID) {
          ...
          if (amount > 0) {
            ...
            stack.shrink(1);
            player.playSound(Sounds.SAW.getSound(), 1, 0.8f + 0.4f * player.getRandom().nextFloat());
          }
        }
        return true;
      }
