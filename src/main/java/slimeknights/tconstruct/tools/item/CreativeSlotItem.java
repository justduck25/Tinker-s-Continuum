...
public class CreativeSlotItem extends Item {
  ...
  public static SlotType getSlot(ItemStack stack) {
    ...
    return nbt.getString(NBT_KEY).map(SlotType::getIfPresent).orElse(null);
  }
  ...
  public void appendHoverText(ItemStack stack, TooltipContext context, TooltipDisplay display, Consumer<Component> tooltip, TooltipFlag flag) {
    SlotType slot = getSlot(stack);
    if (slot != null) {
      tooltip.accept(Component.translatable(TOOLTIP, slot.getDisplayName()).withStyle(ChatFormatting.GRAY));
    } else {
    ...
    if (slotType != null && !toolItem.isEmpty() && toolItem.is(TinkerTags.Items.MODIFIABLE)) {
      if (!player.level().isClientSide() || (player.isCreative() && player.containerMenu.getType() == null)) {
        if (canApply(player)) {
          ...
          int updated = slots.getInt(name).orElse(0) + amount;
          if (updated == 0) {
            slots.remove(name);
          } else {
