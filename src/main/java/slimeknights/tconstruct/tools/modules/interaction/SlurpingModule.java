...
public record SlurpingModule(LevelingValue strength, LevelingInt duration) implements ModifierModule, GeneralInteractionModifierHook, UsingToolModifierHook, KeybindInteractModifierHook, InventoryTickModifierHook, EquipmentChangeModifierHook {
  ...
  private int slurp(FluidStack fluid, ModifierEntry entry, LivingEntity entity, @Nullable Player player, net.neoforged.neoforge.fluids.capability.IFluidHandler.FluidAction action) {
    if (!fluid.isEmpty()) {
  ...
  private static void playDrinkingSound(LivingEntity entity) {
    entity.level().playSound(null, entity.getX(), entity.getY(), entity.getZ(), SoundEvents.GENERIC_DRINK.value(), entity.getSoundSource(), 0.5F, entity.getRandom().nextFloat() * 0.1f + 0.9f);
  }
  ...
  private void finishDrinking(IToolStackView tool, ModifierEntry modifier, LivingEntity entity, boolean playSound) {
    ...
    if (!fluid.isEmpty()) {
      // sound and particles
      if (playSound) {
        playDrinkingSound(entity);
      }
      addFluidParticles(entity, fluid, 16);
      // apply effect
      if (!entity.level().isClientSide()) {
        Player player = asPlayer(entity);
        int consumed = slurp(fluid, modifier, entity, player, EXECUTE);
  ...
  public InteractionResult onToolUse(IToolStackView tool, ModifierEntry modifier, Player player, InteractionHand hand, InteractionSource source) {
    if (source == InteractionSource.RIGHT_CLICK) {
      if (slurp(TANK_HELPER.getFluid(tool), modifier, player, player, SIMULATE) > 0) {
  ...
  public void onUsingTick(IToolStackView tool, ModifierEntry modifier, LivingEntity entity, int useDuration, int timeLeft, ModifierEntry activeModifier) {
    ...
    if (notActive && useTime == 0) {
      FluidStack fluid = TANK_HELPER.getFluid(tool);
      if (!fluid.isEmpty() && slurp(fluid, modifier, entity, asPlayer(entity), SIMULATE) > 0) {
        tool.getPersistentData().putBoolean(modifier.getId().getId(), true);
      }
    }
    ...
    else if (useTime < duration && useTime % 4 == 0 && (!notActive || tool.getPersistentData().getBoolean(modifier.getId().getId()))) {
      FluidStack fluid = TANK_HELPER.getFluid(tool);
      if (!fluid.isEmpty()) {
  ...
  public boolean startInteract(IToolStackView tool, ModifierEntry modifier, Player player, EquipmentSlot slot, TooltipKey keyModifier) {
    if (keyModifier == TooltipKey.NORMAL) {
      if (slurp(TANK_HELPER.getFluid(tool), modifier, player, player, SIMULATE) > 0) {
        tool.getPersistentData().putInt(modifier.getId().getId(), player.tickCount + duration.compute(modifier.getEffectiveLevel()));
        return true;
      }
    }
    return false;
  ...
  public void onInventoryTick(IToolStackView tool, ModifierEntry modifier, Level world, LivingEntity holder, int itemSlot, boolean isSelected, boolean isCorrectSlot, ItemStack stack) {
    if (isCorrectSlot && tool.hasTag(TinkerTags.Items.WORN_ARMOR)) {
      ...
      int finishTime = persistentData.getInt(modifier.getId().getId());
      if (finishTime > 0) {
        ...
        if (timeLeft < 0) {
          ...
          // stop drinking
          persistentData.remove(modifier.getId().getId());
