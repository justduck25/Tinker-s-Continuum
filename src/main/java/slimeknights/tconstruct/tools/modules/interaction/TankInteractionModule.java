...
public record TankInteractionModule(@Nullable InteractionSource source) implements ModifierModule, BlockInteractionModifierHook {
  ...
  public InteractionResult beforeBlockUse(IToolStackView tool, ModifierEntry modifier, UseOnContext context, InteractionSource source) {
    ...
    var fluidHandler = world.getCapability(Capabilities.Fluid.BLOCK, target, face);
    if (fluidHandler == null) {
      return InteractionResult.PASS;
    }
    IFluidHandler cap = IFluidHandler.of(fluidHandler);
    ...
    if (!world.isClientSide()) {
      ...
      if (sneaking) {
        ...
        if (!fluidStack.isEmpty()) {
          int added = cap.fill(fluidStack, EXECUTE);
          if (added > 0) {
        ...
        FluidStack drained = cap.drain(TANK_HELPER.getCapacity(tool), EXECUTE);
        if (!drained.isEmpty()) {
        ...
        FluidStack drained = cap.drain(fluidStack.copyWithAmount(TANK_HELPER.getCapacity(tool) - fluidStack.getAmount()), EXECUTE);
