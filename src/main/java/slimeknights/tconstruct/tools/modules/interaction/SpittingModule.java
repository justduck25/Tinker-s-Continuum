...
public record SpittingModule(LevelingInt shots) implements ModifierModule, GeneralInteractionModifierHook, KeybindInteractModifierHook, InventoryTickModifierHook {
  ...
  /** Writes fluid changes back even if durability damage skipped syncing, such as in creative mode. */
  private static void syncHeldTool(IToolStackView tool, LivingEntity entity) {
    if (tool instanceof ToolStack toolStack) {
      ...
      if (!usedStack.isEmpty() && usedStack.getItem() == tool.getItem()) {
        toolStack.updateStack(usedStack, false);
        return;
      }
  ...
  public boolean startInteract(IToolStackView tool, ModifierEntry modifier, Player player, EquipmentSlot slot, TooltipKey keyModifier) {
    if (!tool.isBroken() && keyModifier == TooltipKey.NORMAL) {
      ...
      if (fluid.getAmount() >= modifier.getLevel() && FluidEffectManager.INSTANCE.find(fluid.getFluid()).hasEffects()) {
        ...
        tool.getPersistentData().putInt(modifier.getId().getId(), player.tickCount + time);
        return true;
      }
    }
    return false;
