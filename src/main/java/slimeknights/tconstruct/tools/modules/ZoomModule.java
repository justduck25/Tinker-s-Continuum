...
public enum ZoomModule implements ModifierModule, GeneralInteractionModifierHook, KeybindInteractModifierHook, UsingToolModifierHook, EquipmentChangeModifierHook {
  ...
  private static void setZoom(ModifierEntry modifier, LivingEntity living, float amount) {
    ...
    if (data != null) {
      data.computeIfAbsent(TinkerDataKeys.FOV_MODIFIER).set(modifier.getId().getId(), amount);
    }
  }

  /** Stops zooming */
  private static void stopZoom(ModifierEntry modifier, Optional<TinkerDataCapability.Holder> tinkerData) {
    tinkerData.ifPresent(data -> data.computeIfAbsent(TinkerDataKeys.FOV_MODIFIER).remove(modifier.getId().getId()));
  }

  /** Stops zooming */
  private static void stopZoom(ModifierEntry modifier, LivingEntity entity) {
    stopZoom(modifier, Optional.ofNullable(TinkerDataCapability.getData(entity)));
  }
