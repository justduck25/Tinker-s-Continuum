...
public enum SmashingModule implements ModifierModule, FluidModifierHook, ProjectileLaunchModifierHook.NoShooter, ProjectileHitModifierHook, ProjectileFuseModifierHook, VolatileDataModifierHook, ValidateModifierHook, ModifierRemovalHook, DisplayNameModifierHook, TooltipModifierHook {
  ...
  /** Gets the current fluid component patch. */
  private static DataComponentPatch getFluidPatch(IModDataView data) {
    if (data.contains(KEY_FLUID_TAG, Tag.TAG_COMPOUND)) {
      return data.getCompound(KEY_FLUID_TAG).read("patch", DataComponentPatch.CODEC).orElse(DataComponentPatch.EMPTY);
    }
    return DataComponentPatch.EMPTY;
  }

  /** Serializes a fluid component patch into persistent data. */
  private static CompoundTag serializeFluidPatch(DataComponentPatch patch) {
    CompoundTag tag = new CompoundTag();
    tag.store("patch", DataComponentPatch.CODEC, patch);
    return tag;
  }

  /** Creates a stored fluid stack with its component patch. */
  private static FluidStack storedFluid(Fluid fluid, int amount, IModDataView data) {
    return new FluidStack(fluid, amount, getFluidPatch(data));
  }
  ...
  @Override
  public int fill(IToolStackView tool, ModifierEntry modifier, FluidStack resource, net.neoforged.neoforge.fluids.capability.IFluidHandler.FluidAction action) {
    if (resource.isEmpty()) {
      return 0;
    }
    ...
    if (action.execute()) {
      ...
      if (!patch.isEmpty()) {
        data.put(KEY_FLUID_TAG, serializeFluidPatch(patch));
      }
    }
    return amount;
  }
  ...
  @Override
  public FluidStack drain(IToolStackView tool, ModifierEntry modifier, int maxDrain, net.neoforged.neoforge.fluids.capability.IFluidHandler.FluidAction action) {
    if (maxDrain > 0) {
      ModDataNBT data = tool.getPersistentData();
      ...
      if (fluid != Fluids.EMPTY) {
        ...
        if (amount <= 0) {
          ...
          FluidStack result = storedFluid(fluid, amount, data);
          if (action.execute()) {
            clearFluid(data);
          }
          return result;
  ...
  @Override
  public FluidStack drain(IToolStackView tool, ModifierEntry modifier, FluidStack resource, net.neoforged.neoforge.fluids.capability.IFluidHandler.FluidAction action) {
    if (!resource.isEmpty()) {
      ModDataNBT data = tool.getPersistentData();
      Fluid fluid = getFluid(data);
      ...
      if (fluid != Fluids.EMPTY && resource.getFluid() == fluid) {
        ...
        if (amount <= 0) {
          ...
          DataComponentPatch storedPatch = getFluidPatch(data);
          if (Objects.equals(storedPatch, resource.getComponentsPatch())) {
            FluidStack result = new FluidStack(fluid, amount, storedPatch);
            if (action.execute()) {
              clearFluid(data);
            }
            return result;
  ...
  public FluidStack getFluidInTank(IToolStackView tool, ModifierEntry modifier, int tank) {
    ...
    if (fluid != Fluids.EMPTY) {
      int amount = getAmount(modifier, fluid);
      if (amount > 0) {
        return storedFluid(fluid, amount, data);
      } else {
      ...
      if (amount > 0) {
        ...
        if (effects.hasEntityEffects()) {
          ...
          int drained = effects.applyToEntity(
            storedFluid(fluid, amount, persistentData),
  ...
  public Component getDisplayName(IToolStackView tool, ModifierEntry entry, Component name, @Nullable RegistryAccess access) {
    ...
    if (fluid != Fluids.EMPTY) {
      ...
      return Component.translatable(FORMAT, name,
        storedFluid(fluid, FluidValues.BOTTLE, data).getHoverName()
  ...
  public void addTooltip(IToolStackView tool, ModifierEntry modifier, @Nullable Player player, List<Component> tooltip, TooltipKey tooltipKey, TooltipFlag tooltipFlag) {
    ...
    if (fluid != Fluids.EMPTY) {
      ...
      if (amount > 0) {
        ...
        tooltip.add(modifier.getModifier().applyStyle(storedFluid(fluid, amount, data).getHoverName().copy()
