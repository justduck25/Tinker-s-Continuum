...
public enum SleevesModule implements ModifierModule, GeneralInteractionModifierHook, InventorySelectionModule {
  ...
  public void onInventorySelect(IToolStackView tool, ModifierEntry modifier, Player player, int newIndex, ItemStack stack) {
    Component message = Component.translatable(SELECTED, stack.getHoverName(), newIndex + 1);
  ...
  public void onDisableSelection(IToolStackView tool, ModifierEntry modifier, Player player) {
    if (player instanceof ServerPlayer serverPlayer) {
      serverPlayer.sendSystemMessage(DISABLED, true);
    } else {
      player.sendSystemMessage(DISABLED);
    }
  }
      ...
      if (!player.isCrouching()) {
        ...
        if (!ammo.isEmpty()) {
          if (!player.getCooldowns().isOnCooldown(ammo)) {
            ...
            // handle result
            inventory.setStack(tool, modifier, selected, result instanceof InteractionResult.Success success && success.heldItemTransformedTo() != null ? success.heldItemTransformedTo() : ammo);
            return result;
          } else {
