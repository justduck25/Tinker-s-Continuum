  ...
  public void processLoot(IToolStackView tool, ModifierEntry modifier, List<ItemStack> generatedLoot, LootContext context) {
    ...
    if (entity != null) {
      ...
      if (generatedLoot.stream().noneMatch(stack -> stack.is(ItemTags.SKULLS))) {
        ...
        List<SeveringRecipe> recipes = SeveringRecipeCache.findRecipe(world.recipeAccess(), entity.getType());
        if (!recipes.isEmpty()) {
          float level = modifier.getEffectiveLevel();
          float looting = context.getOptionalParameter(LootContextParams.ENCHANTMENT_LEVEL) == null ? 0 : context.getOptionalParameter(LootContextParams.ENCHANTMENT_LEVEL);
