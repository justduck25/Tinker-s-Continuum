...
public class TrimModule implements ModifierModule, DisplayNameModifierHook, ModifierRemovalHook {
  ...
  public Component getDisplayName(IToolStackView tool, ModifierEntry entry, Component name, @Nullable RegistryAccess access) {
    ...
    if (formatted == null) {
      ...
      formatted = original;
      TrimMaterial material = access.lookupOrThrow(Registries.TRIM_MATERIAL).get(Identifier.tryParse(trimMaterial)).map(Holder::value).orElse(null);
      // if pattern is not passed, use the modifier name directly. Lets us trim items without patterns
      TrimPattern pattern = trimPattern.isEmpty() ? null : access.lookupOrThrow(Registries.TRIM_PATTERN).get(Identifier.tryParse(trimPattern)).map(Holder::value).orElse(null);
