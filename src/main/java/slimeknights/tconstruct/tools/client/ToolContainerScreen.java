...
public class ToolContainerScreen extends AbstractContainerScreen<ToolContainerMenu> implements IScreenWithFluidTank {
  ...
  private static final int INV_HEIGHT = 96;
  ...
  public ToolContainerScreen(ToolContainerMenu menu, Inventory inv, Component title) {
    super(menu, inv, title, 176, calcHeight(menu));
    ...
    if (tank.getCapacity() > 0) {
      this.tank = new GuiTankModule(this, tank, 8, this.imageHeight - INV_HEIGHT - 9, 160, 8, true, null);
    } else {
      this.tank = null;
    }
    if (slots > 0) {
  ...
  /** Calculates the full screen height before calling the final-sized MC 26 screen constructor. */
  private static int calcHeight(ToolContainerMenu menu) {
    int slots = menu.getItemHandler().getSlots();
    if (menu.isShowOffhand()) {
      slots++;
    }
    ...
    int craftingHeight = menu.getCraftingHeight() * SLOT_SIZE;
    int tankHeight = menu.getTank().getCapacity() > 0 ? FLUID_TANK.h : 0;
    return UI_START + TITLE_SIZE + INV_HEIGHT + inventoryRows * SLOT_SIZE + craftingHeight + tankHeight;
  }
  @Override
  protected void slotClicked(Slot slot, int slotId, int buttonNum, ContainerInput containerInput) {
    ...
    if (containerInput == ContainerInput.SWAP && slot.container == menu.getPlayer().getInventory() && slot.getSlotIndex() == menu.getSlotIndex()) {
      return;
    }
  ...
  public void extractBackground(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTicks) {
    ...
    graphics.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, xStart, yStart + slotBackground, 0, PLAYER_INVENTORY_START, this.imageWidth, INV_HEIGHT, 256, 256);
    ...
    if (slotIndex < 9) {
      ...
      // offhand is the last slot, but only if the offhand is shown in the inventory
    ...
    TextureAtlas blocksAtlas = this.minecraft.getAtlasManager().getAtlasOrThrow(AtlasIds.BLOCKS);
    ...
    for (int modIndex = modifiers.size() - 1; modIndex >= 0; modIndex--) {
      ...
      for (int i = 0; i < size; i++) {
        ...
        if (pattern != null) {
          ...
          graphics.blitSprite(RenderPipelines.GUI_TEXTURED, sprite, xStart + slot.x, yStart + slot.y, 16, 16);
        }
      }
      start += size;
    }
    ...
    if (menu.isShowOffhand()) {
      ...
      if (!slot.hasItem()) {
        TextureAtlasSprite sprite = blocksAtlas.getSprite(Patterns.SHIELD.getTexture());
        graphics.blitSprite(RenderPipelines.GUI_TEXTURED, sprite, xStart + slot.x, yStart + slot.y, 16, 16);
      }
    }
  ...
  public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
    ...
    if (tank != null && (event.button() == 0 || event.button() == 1) && !menu.getCarried().isEmpty() && !minecraft.player.isSpectator()) {
      if (tank.tryClick((int)event.x() - leftPos, (int)event.y() - topPos, event.button(), 0)) {
        return true;
      }
    }
