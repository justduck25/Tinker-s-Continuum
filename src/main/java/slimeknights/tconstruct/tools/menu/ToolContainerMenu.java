...
public class ToolContainerMenu extends AbstractContainerMenu {
  ...
  /** Creates a minimal client-side tool container from the currently selected stack. */
  public static ToolContainerMenu forClient(int id, Inventory inventory) {
    int slot = inventory.getSelectedSlot();
    ItemStack stack = inventory.getItem(slot);
    int size = ModifierUtil.getVolatileInt(stack, ToolInventoryCapability.TOTAL_SLOTS);
    IItemHandler handler = new ItemStackHandler(size);
    return new ToolContainerMenu(TinkerTools.toolContainer.get(), id, inventory, stack, handler, slot);
  }
  ...
  protected ToolContainerMenu(@Nullable MenuType<?> type, int id, Inventory playerInventory, ItemStack stack, IItemHandler handler, int slotIndex, CraftingType craftingType, boolean includeOffhand) {
    ...
    this.tank = new ToolFluidHandler(stack, tool, player.level().isClientSide() ? null : player);
  ...
  public void slotsChanged(Container pContainer) {
    ...
    if (craftingContainer != null && resultContainer != null && player.level() instanceof ServerLevel level && player instanceof ServerPlayer serverPlayer) {
      CraftingInput input = craftingContainer.asCraftInput();
      ItemStack result = ItemStack.EMPTY;
      Optional<RecipeHolder<CraftingRecipe>> recipe = level.getServer().getRecipeManager().getRecipeFor(RecipeType.CRAFTING, input, level, (RecipeHolder<CraftingRecipe>) null);
      if (recipe.isPresent()) {
    ...
    if ((id == 0 || id == 1) && !held.isEmpty() && !player.isSpectator()) {
      if (!player.level().isClientSide()) {
        ...
        if (result != null) {
          ...
          syncTankToClient(player);
        } else {
          setCarried(interactWithBucket(player, held, id));
        }
      }
      return true;
    }
  ...
  /** Fallback for vanilla buckets in NeoForge transfer gaps. */
  private ItemStack interactWithBucket(Player player, ItemStack held, int id) {
    if (held.getItem() instanceof BucketItem bucketItem) {
      if (id == 1 && bucketItem.getContent() != Fluids.EMPTY) {
        FluidStack fluid = new FluidStack(bucketItem.getContent(), FluidType.BUCKET_VOLUME);
          if (tank.fill(fluid, SIMULATE) == FluidType.BUCKET_VOLUME) {
            tank.fill(fluid, EXECUTE);
            syncTankToClient(player);
            return ItemUtils.createFilledResult(held, player, new ItemStack(Items.BUCKET));
          }
      }
      if (id == 0 && bucketItem.getContent() == Fluids.EMPTY) {
        FluidStack contained = tank.getFluid().copyWithAmount(FluidType.BUCKET_VOLUME);
        if (!contained.isEmpty() && tank.drain(contained, SIMULATE).getAmount() == FluidType.BUCKET_VOLUME) {
          ItemStack filled = new ItemStack(contained.getFluid().getBucket());
          if (!filled.isEmpty() && !filled.is(Items.AIR)) {
            tank.drain(contained, EXECUTE);
            syncTankToClient(player);
            return ItemUtils.createFilledResult(held, player, filled);
          }
        }
      }
    }
  ...
  private void syncTankToClient(Player player) {
    if (player instanceof ServerPlayer serverPlayer) {
      TinkerNetwork.getInstance().sendTo(new ToolContainerFluidUpdatePacket(tank.getFluid()), serverPlayer);
    }
  }
  ...
  private record ToolFluidHandler(ItemStack stack, IToolStackView tool, @Nullable Player player) implements SimpleFluidTank {
    @Nonnull
    @Override
    ...
    public void updateFluid(FluidStack updated, int change) {
      if (change != 0) {
        setFluid(updated);
        if (player != null) {
          TinkerNetwork.getInstance().sendTo(new ToolContainerFluidUpdatePacket(updated), (ServerPlayer) player);
        }
      }
    }
