...
public class InteractionHandler {
  ...
  private static InteractionResult tryPiggyBackPackPickup(Player player, Entity target, InteractionHand hand) {
    if (player.isSpectator() || !(target instanceof LivingEntity livingTarget)) {
      return InteractionResult.PASS;
    }
    ItemStack chestplate = player.getItemBySlot(EquipmentSlot.CHEST);
    if (chestplate.getItem() instanceof PiggyBackPackItem backpack) {
      return backpack.interactLivingEntity(chestplate, player, livingTarget, hand);
    }
    return InteractionResult.PASS;
  }

  @SubscribeEvent
  static void piggyBackpackDropOnTick(EntityTickEvent.Post event) {
    if (!event.getEntity().level().isClientSide() && event.getEntity() instanceof Player player && player.isVehicle()) {
      ItemStack chestplate = player.getItemBySlot(EquipmentSlot.CHEST);
      int maxCount = chestplate.getItem() == TinkerGadgets.piggyBackpack.get() ? chestplate.getCount() : 0;
      int oldCount = player.getPassengers().size();
      TinkerGadgets.piggyBackpack.get().matchCarriedEntitiesToCount(player, maxCount);
      if (oldCount != player.getPassengers().size() && player instanceof ServerPlayer serverPlayer) {
  ...
  static void afterEntityInteractSpecific(EntityInteractSpecific event) {
    Player player = event.getEntity();
    if (event.getItemStack().isEmpty() && !player.isSpectator()) {
      InteractionResult piggyResult = tryPiggyBackPackPickup(player, event.getTarget(), event.getHand());
      if (piggyResult.consumesAction()) {
        event.setCanceled(true);
        event.setCancellationResult(piggyResult);
      }
    }
  }
  ...
  static void onBlock(LivingShieldBlockEvent event) {
    ...
    if (!activeStack.isEmpty() && activeStack.is(TinkerTags.Items.MODIFIABLE)) {
      ...
      if (!tool.isBroken() && canBlock(entity, event.getDamageSource().getSourcePosition(), tool)) {
