...
/** Material stat class handling armor plating and shields. */
public record PlatingMaterialStats(MaterialStatType<?> getType, int durability, float armor, float toughness, float knockbackResistance) implements IRepairableMaterialStats.ScaledTooltip {
  ...
  public static final MaterialStatType<PlatingMaterialStats> SHIELD = new MaterialStatType<>(new MaterialStatsId(TConstruct.MOD_ID, "plating_shield"), (java.util.function.Function<MaterialStatType<PlatingMaterialStats>, PlatingMaterialStats>) type -> new PlatingMaterialStats(type, 1, 0, 0, 0), RecordLoadable.create(
  ...
  /** All armor plating types in ArmorType ordinal order, plus shield last. */
  ...
  public List<Component> getLocalizedInfo(float scale) {
    ...
    Component knockbackResistance = ToolStats.KNOCKBACK_RESISTANCE.formatValue(this.knockbackResistance * 10 * scale);
    if (getType == SHIELD) {
  ...
  private static MaterialStatType<PlatingMaterialStats> makeType(String name) {
    return new MaterialStatType<>(new MaterialStatsId(TConstruct.MOD_ID, name), (java.util.function.Function<MaterialStatType<PlatingMaterialStats>, PlatingMaterialStats>) type -> new PlatingMaterialStats(type, 1, 0, 0, 0), LOADABLE);
  }
  ...
  /** Builder to create plating material stats for all armor pieces and shields. */
  @Setter
  ...
  public static class Builder implements ArmorShieldModuleBuilder<PlatingMaterialStats> {
    ...
    /** Sets the armor value for each piece in boots, leggings, chestplate, helmet order. */
