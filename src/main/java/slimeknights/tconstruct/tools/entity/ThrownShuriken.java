...
public class ThrownShuriken extends Projectile implements ToolProjectile, ProjectileWithPower, ProjectileWithKnockback, ReusableProjectile {
  ...
  public void tick() {
    ...
    if (hit.getType() == HitResult.Type.BLOCK) {
      ...
      if (state.is(Blocks.NETHER_PORTAL)) {
        ...
        if (this.level() instanceof ServerLevel serverLevel && this.level().getBlockEntity(pos) instanceof TheEndGatewayBlockEntity gateway) {
          Vec3 portalPosition = gateway.getPortalPosition(serverLevel, pos);
          if (portalPosition != null) {
            this.setPos(portalPosition);
            TheEndGatewayBlockEntity.triggerCooldown(serverLevel, pos, state, gateway);
          }
        }
        teleported = true;
      }
    ...
    this.applyEffectsFromBlocks(this.position(), new Vec3(x, y, z));
    this.updateRotation();
  ...
  protected void onHitBlock(BlockHitResult result) {
    ...
    if (!this.isRemoved() && level() instanceof ServerLevel serverLevel) {
      this.spawnAtLocation(serverLevel, stack.copy());
      this.discard();
    }
  }
  ...
  protected void readAdditionalSaveData(ValueInput input) {
    ...
    input.read(KEY_STACK, ItemStack.CODEC).ifPresent(this::setStack);
    this.entityData.set(WATER_INERTIA, input.getFloatOr(KEY_WATER_INERTIA, 0.8f));
