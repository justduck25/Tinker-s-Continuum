...
public class CombatFishingHook extends FishingHook implements ProjectileWithKnockback, ProjectileWithPower {
  ...
  /** Age while stuck in a wall. Replaces the vanilla private life field for grapple expiry. */
  ...
  private void pullGrapple(Entity owner) {
    ...
    if (isDrill() && owner instanceof Player player) {
      player.startAutoSpinAttack(20, 1.0f, player.getMainHandItem());
    }
  ...
  public void readAdditionalSaveData(ValueInput input) {
    ...
    String materialString = input.getStringOr(TAG_MATERIAL, "");
    if (!materialString.isEmpty()) {
      setMaterial(Objects.requireNonNullElse(MaterialVariantId.tryParse(materialString), IMaterial.UNKNOWN_ID));
    }
  }
}
