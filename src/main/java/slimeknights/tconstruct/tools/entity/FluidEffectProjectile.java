...
public class FluidEffectProjectile extends Projectile implements ProjectileWithKnockback, ProjectileWithPower {
  ...
  private IItemHandlerModifiable getCannonInventory() {
    ...
    if (this.cannon != null && level.isLoaded(this.cannon)) {
      var handler = level.getCapability(Capabilities.Item.BLOCK, this.cannon, null);
  ...
  protected void onHitEntity(EntityHitResult result) {
    ...
    if (!level.isClientSide() && !fluid.isEmpty()) {
      ...
      if (recipe.hasEntityEffects()) {
        ...
        int consumed = recipe.applyToEntity(fluid, power, context, EXECUTE);
        // update the stack
        if (consumed > 0) {
    ...
    if (!level.isClientSide()) {
      ...
      if (!fluid.isEmpty()) {
        ...
        if (recipe.hasBlockEffects()) {
          ...
          int consumed = recipe.applyToBlock(fluid, power, context, EXECUTE);
          ...
          if (!fluid.isEmpty()) {
            ...
            while (iterator.hasNext() && !fluid.isEmpty()) {
              consumed = recipe.applyToBlock(fluid, power, context.withHitResult(Util.offset(hitResult, hit.relative(iterator.next().getOpposite()))), EXECUTE);
              fluid.shrink(consumed);
              changed |= consumed > 0;
            }
          }
  ...
  protected void addAdditionalSaveData(ValueOutput output) {
    ...
    output.putBoolean(KEY_CANNON + "Present", cannon != null);
    if (cannon != null) {
      output.putInt(KEY_CANNON + "X", cannon.getX());
      output.putInt(KEY_CANNON + "Y", cannon.getY());
      output.putInt(KEY_CANNON + "Z", cannon.getZ());
    }
  }
  ...
  protected void readAdditionalSaveData(ValueInput input) {
    ...
    this.power = input.getFloatOr(KEY_POWER, 1f);
    this.knockback = input.getFloatOr(KEY_KNOCKBACK, 1f);
    this.entityData.set(WATER_INERTIA, input.getFloatOr(KEY_WATER_INERTIA, 0.6f));
    if (input.getBooleanOr(KEY_CANNON + "Present", false)) {
      this.cannon = new BlockPos(input.getIntOr(KEY_CANNON + "X", 0), input.getIntOr(KEY_CANNON + "Y", 0), input.getIntOr(KEY_CANNON + "Z", 0));
    } else {
      this.cannon = null;
    }
  }
