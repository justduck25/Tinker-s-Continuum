...
public abstract class AbstractMeltingCategory implements IRecipeCategory<MeltingRecipe> {
  ...
  /** Static outline/frame; animated drawable only reveals the filled portion. */
  protected final IDrawableStatic arrowFrame;
  ...
  public AbstractMeltingCategory(IGuiHelper helper) {
    ...
    this.arrowFrame = helper.drawableBuilder(BACKGROUND_LOC, 56, 18, 24, 17).build();
  ...
  @Override
  public int getWidth() { return 132; }
  @Override
...
@Override
  public void draw(MeltingRecipe recipe, IRecipeSlotsView slots, GuiGraphicsExtractor graphics, double mouseX, double mouseY) {
    // Draw fixed outline first; animated drawable clips the unfilled part.
    arrowFrame.draw(graphics, 56, 18);
  ...
  public void getTooltip(ITooltipBuilder tooltip, MeltingRecipe recipe, IRecipeSlotsView slots, double mouseXD, double mouseYD) {
    ...
    } else if (GuiUtil.isHovered(mouseX, mouseY, 56, 18, 24, 17)) {
  ...
  @Override
  public Identifier getIdentifier(MeltingRecipe recipe) {
    return recipe.getId();
  }
}
