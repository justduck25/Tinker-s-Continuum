...
public class MeltingCategory extends AbstractMeltingCategory {
  ...
  private static final IRecipeSlotRichTooltipCallback ITEM_FUEL_TOOLTIP = (slot, list) -> {
    MeltingFuel solid = MeltingFuelLookup.getSolid();
    list.add(Component.translatable(KEY_TEMPERATURE, solid.getTemperature()).withStyle(ChatFormatting.GRAY));
    list.add(Component.translatable(KEY_MULTIPLIER, solid.getRate() / 10f).withStyle(ChatFormatting.GRAY));
  };
  ...
  public void setRecipe(IRecipeLayoutBuilder builder, MeltingRecipe recipe, IFocusGroup focuses) {
    // input
    List<ItemStack> displayItems = MaterialRecipeCache.getDisplayItems(recipe.getInput());
    if (displayItems.isEmpty()) {}
    builder.addSlot(RecipeIngredientRole.INPUT, 24, 18).addItemStacks(displayItems);
