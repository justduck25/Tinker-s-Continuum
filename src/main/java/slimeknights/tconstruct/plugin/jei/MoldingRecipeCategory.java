...
public class MoldingRecipeCategory implements IRecipeCategory<MoldingRecipe> {
  ...
  public void draw(MoldingRecipe recipe, IRecipeSlotsView slots, GuiGraphicsExtractor graphics, double mouseX, double mouseY) {
    ...
    if (!MaterialRecipeCache.getDisplayItems(recipe.getPattern()).isEmpty()) {
  ...
  public void getTooltip(ITooltipBuilder tooltip, MoldingRecipe recipe, IRecipeSlotsView slots, double mouseX, double mouseY) {
    if (recipe.isPatternConsumed() && !MaterialRecipeCache.getDisplayItems(recipe.getPattern()).isEmpty() && GuiUtil.isHovered((int)mouseX, (int)mouseY, 50, 7, 18, 18)) {
  ...
  public void setRecipe(IRecipeLayoutBuilder builder, MoldingRecipe recipe, IFocusGroup focuses) {
    // basic input output
    builder.addSlot(RecipeIngredientRole.INPUT, 3, 24).addItemStacks(MaterialRecipeCache.getDisplayItems(recipe.getMaterial()));
    RegistryAccess access = SafeClientAccess.getRegistryAccess();
    if (access != null) {
      builder.addSlot(RecipeIngredientRole.OUTPUT, 51, 24).addItemStack(recipe.assemble(null));
    }
    ...
    if (!MaterialRecipeCache.getDisplayItems(pattern).isEmpty()) {
      IRecipeSlotBuilder inputSlot = builder.addSlot(RecipeIngredientRole.INPUT, 3, 1).addItemStacks(MaterialRecipeCache.getDisplayItems(pattern));
      if (!recipe.isPatternConsumed()) {
        IRecipeSlotBuilder preservedSlot = builder.addSlot(RecipeIngredientRole.RENDER_ONLY, 51, 8).addItemStacks(MaterialRecipeCache.getDisplayItems(pattern));
  ...
  @Override
  public Identifier getIdentifier(MoldingRecipe recipe) {
    return recipe.getId();
  }
}
