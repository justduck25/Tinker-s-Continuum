...
public class GreyToColorMapping implements IColorMapping {
  ...
  public JsonObject serialize(JsonSerializationContext context) {
    ...
    for (ColorMapping mapping : mappings) {
      ...
      pair.addProperty("color", String.format("%08X", mapping.color));
      colors.add(pair);
    }
  ...
  public static int scaleColor(int original, int newColor, int grey) {
    ...
    int newRed = red(newColor);
    if (red < grey) newRed = newRed * red / grey;
    int green = green(original);
    int newGreen = green(newColor);
    if (green < grey) newGreen = newGreen * green / grey;
    int blue = blue(original);
    int newBlue = blue(newColor);
    if (blue < grey) newBlue = newBlue * blue / grey;

    // final color
    return color(alpha(newColor), newRed, newGreen, newBlue);
  }

  /* ARGB32 helper methods */

  /** Extracts alpha from ARGB */
  private static int alpha(int c) { return c >>> 24; }
  /** Extracts red from ARGB */
  private static int red(int c) { return (c >> 16) & 0xFF; }
  /** Extracts green from ARGB */
  private static int green(int c) { return (c >> 8) & 0xFF; }
  /** Extracts blue from ARGB */
  private static int blue(int c) { return c & 0xFF; }
  /** Packs ARGB color */
  private static int color(int a, int r, int g, int b) { return (a << 24) | (r << 16) | (g << 8) | b; }
