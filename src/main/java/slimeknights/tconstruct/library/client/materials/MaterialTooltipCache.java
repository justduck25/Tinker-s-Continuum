...
public class MaterialTooltipCache {
  ...
  public static void init(AddClientReloadListenersEvent manager)  {
    manager.addListener(Identifier.fromNamespaceAndPath("tconstruct", "material_tooltip_cache"), RELOAD_LISTENER);
  }
  ...
  private static final Function<MaterialVariantId,MutableComponent> DISPLAY_NAME_GETTER = id -> {
    ...
    return Component.translatable(getKey(id.getMaterialId()));
  };
  ...
  public static final Function<MaterialVariantId,TextColor> COLOR_GETTER = id -> {
    ...
    return ResourceColorManager.getTextColor(getKey(id.getMaterialId()));
  };
