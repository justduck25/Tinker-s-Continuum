...
public class FluidModifierModel extends NormalModifierModel {
  ...
  public Object getCacheKey(IToolStackView tool, ModifierEntry entry) {
    ...
    if (!fluid.isEmpty()) {
      return new FluidModifierCacheKey(entry.getModifier(), fluid.getFluid(), fluid.getComponentsPatch());
    }
  ...
  public void addQuads(IToolStackView tool, ModifierEntry entry, Function<Material,TextureAtlasSprite> spriteGetter, Transformation transforms, boolean isLarge, int startTintIndex, Consumer<Collection<BakedQuad>> quadConsumer, @Nullable ItemLayerPixels pixels) {
    ...
    if (!fluid.isEmpty()) {
      ...
      if (template != null) {
        slimeknights.tconstruct.library.client.modifiers.model.FluidModifierModel.addQuads(fluid, template, spriteGetter, transforms, quadConsumer, pixels);
      }
    }
  ...
  private record FluidModifierCacheKey(Modifier modifier, Fluid fluid, DataComponentPatch components) {}
