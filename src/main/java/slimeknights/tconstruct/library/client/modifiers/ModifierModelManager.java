...
public class ModifierModelManager implements IEarlySafeManagerReloadListener {
  ...
  public static void init(AddClientReloadListenersEvent manager) {
    manager.addListener(Identifier.fromNamespaceAndPath("tconstruct", "modifier_models"), INSTANCE);
  }
  ...
  public void onReloadSafe(ResourceManager manager) {
    ...
    if (!eventFired) {
      TConstruct.MOD_EVENT_BUS.post(new ModifierModelRegistrationEvent());
      eventFired = true;
    }
  ...
  private static Material getModifierTexture(Identifier modifierRoot, Identifier modifierId, String suffix) {
    return ModifierModel.blockAtlas(Identifier.fromNamespaceAndPath(modifierRoot.getNamespace(), modifierRoot.getPath() + modifierId.getNamespace() + "_" + modifierId.getPath() + suffix));
  }
    ...
    for (Entry<ModifierId, IUnbakedModifierModel> entry : modifierModels.entrySet()) {
      ...
      if (!skip.contains(id) && !blacklist.contains(model)) {
        IBakedModifierModel toolModel = model.forTool(
          name -> getTexture(smallModifierRoots, validator, id.getId(), name),
          name -> getTexture(largeModifierRoots, validator, id.getId(), name));
        if (toolModel != null) {
