...
// TODO: 1.21.4 - ArmorItem was removed
...
public class ContentMaterialSkull extends AbstractMaterialContent {
  ...
  private IDisplayableCastingRecipe getSkullRecipe() {
    ...
    if (!searchedSkullRecipe && world != null) {
      ...
      skullRecipe = manager != null
        ? RecipeHelper.getRecipes(manager, TinkerRecipeTypes.CASTING_BASIN.get(), slimeknights.tconstruct.library.recipe.casting.ICastingRecipe.class).stream()
          .filter(recipe -> recipe instanceof IDisplayableCastingRecipe)
          .map(recipe -> (IDisplayableCastingRecipe) recipe)
          .filter(recipe -> recipe.getOutputs().stream().anyMatch(output -> output.getItem() == TinkerTools.slimesuit.get(ArmorType.HELMET) && MaterialIdNBT.from(output).getMaterial(0).getId().toString().equals(materialName)))
          .findFirst()
          .orElse(null)
        : null;
