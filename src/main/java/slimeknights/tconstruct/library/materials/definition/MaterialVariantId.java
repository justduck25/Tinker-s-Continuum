...
public sealed interface MaterialVariantId permits MaterialId, MaterialVariantIdImpl {
  ...
  EntityDataSerializer<MaterialVariantId> DATA_ACCESSOR = EntityDataSerializer.forValueType(StreamCodec.of((FriendlyByteBuf buf, MaterialVariantId value) -> value.toNetwork(buf), MaterialVariantId::fromNetwork));
  ...
  /** Gets the material ID as a {@link MaterialId} */
  default MaterialId getMaterialId() {
    return new MaterialId(getId());
  }
