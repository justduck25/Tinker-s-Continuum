...
public class ModifiableBowItem extends ModifiableLauncherItem {
  ...
  public InteractionResult use(Level level, Player player, InteractionHand hand) {
    ...
    if (!ammo.isEmpty()) {
      if (storeDrawingItem) {
        tool.getPersistentData().put(KEY_DRAWBACK_AMMO, slimeknights.tconstruct.library.utils.ItemStackNbtHelper.save(ammo));
      } else {
  ...
  public boolean releaseUsing(ItemStack bow, Level level, LivingEntity living, int timeLeft) {
    ...
    if (level instanceof ServerLevel serverLevel) {
      ...
      for (int arrowIndex = 0; arrowIndex < ammo.getCount(); arrowIndex++) {
        ...
        ThrownTool thrown = null;
        if (thrownTool) {
          thrown = new ThrownTool(level, living, ammo, charge, velocity, waterInertia);
        ...
        projectile.shootFromRotation(living, living.getXRot() + angle, living.getYRot(), 0, power * 3.0F, inaccuracy);
        if (arrow != null) {
          if (charge == 1.0F) {
        ...
        for (ModifierEntry entry : modifiers.getModifiers()) {
          entry.getHook(ModifierHooks.PROJECTILE_LAUNCH).onProjectileLaunch(tool, entry, living, ammo, projectile, arrow, arrowData, arrowIndex == primaryIndex);
        }

        // allow ballista to run a few remaining hooks if present
        if (thrownTool) {
          if (thrown != null) {
            thrown.onRelease(living, arrowData);
          }
        }
