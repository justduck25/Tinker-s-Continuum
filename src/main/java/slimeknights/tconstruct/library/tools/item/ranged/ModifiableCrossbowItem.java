...
public class ModifiableCrossbowItem extends ModifiableLauncherItem {
  ...
  public InteractionResult use(Level level, Player player, InteractionHand hand) {
    ...
    if (heldAmmo.isEmpty()) {
      ...
      if (!ammo.isEmpty() || tool.getModifiers().has(TinkerTags.Modifiers.CHARGE_EMPTY_BOW_WITH_DRAWTIME)) {
        ...
        if (!ammo.isEmpty()) {
          if (storeDrawingItem) {
            persistentData.put(KEY_DRAWBACK_AMMO, slimeknights.tconstruct.library.utils.ItemStackNbtHelper.save(ammo));
          } else {
    ...
    if (sinistral) {
      ...
      if (ModifierUtil.canPerformAction(tool, net.neoforged.neoforge.common.ItemAbility.get("shield_block"))) {
        player.startUsingItem(hand);
        return InteractionResult.CONSUME;
      }
    }
  ...
  public boolean releaseUsing(ItemStack bow, Level level, LivingEntity living, int chargeRemaining) {
    ...
    if (!ammo.isEmpty()) {
      ...
      if (level instanceof ServerLevel serverLevel) {
        CompoundTag ammoNBT = slimeknights.tconstruct.library.utils.ItemStackNbtHelper.save(ammo);
  ...
  public static void fireCrossbow(IToolStackView tool, LivingEntity living, boolean creative, InteractionHand hand, CompoundTag heldAmmo) {
    ...
    if (level instanceof ServerLevel serverLevel) {
      ...
      ItemStack ammo = slimeknights.tconstruct.library.utils.ItemStackNbtHelper.parse(heldAmmo);
      if (ammo.isEmpty()) {
        ammo = new ItemStack(Items.ARROW);
      }
      int projectileCount = Math.max(1, ammo.getCount());
      float startAngle = getAngleStart(projectileCount);
      int primaryIndex = projectileCount / 2;
      for (int arrowIndex = 0; arrowIndex < projectileCount; arrowIndex++) {
        ItemStack projectileAmmo = ammo.copyWithCount(1);
        // setup projectile
        AbstractArrow arrow = null;
        Projectile projectile;
        float speed;
        if (projectileAmmo.is(Items.FIREWORK_ROCKET)) {
          // TODO: don't hardcode fireworks, perhaps use a map or a JSON behavior list
          projectile = new FireworkRocketEntity(level, projectileAmmo, living, living.getX(), living.getEyeY() - 0.15f, living.getZ(), true);
          speed = 1.5f;
          damage += 3;
        } else {
          ArrowItem arrowItem = projectileAmmo.getItem() instanceof ArrowItem a ? a : (ArrowItem)Items.ARROW;
          arrow = arrowItem.createArrow(level, projectileAmmo, living, living.getItemInHand(hand));
          projectile = arrow;
        ...
        // setup projectile. Match original TConstruct and vanilla MC 26.1 crossbow behavior: rotate the view vector around the shooter up-vector.
        float angle = startAngle + (10 * arrowIndex);
        Vec3 upVector = living.getUpVector(1.0F);
        Quaternionf upQuaternion = new Quaternionf().setAngleAxis(angle * Math.PI / 180F, upVector.x, upVector.y, upVector.z);
        Vector3f targetVector = living.getViewVector(1.0F).toVector3f().rotate(upQuaternion);
        ...
        for (ModifierEntry entry : modifiers.getModifiers()) {
          entry.getHook(ModifierHooks.PROJECTILE_LAUNCH).onProjectileLaunch(tool, entry, living, projectileAmmo, projectile, arrow, projectileData, arrowIndex == primaryIndex);
        }
  ...
  public List<Component> getStatInformation(IToolStackView tool, @Nullable Player player, List<Component> tooltips, TooltipKey key, TooltipFlag tooltipFlag) {
    ...
    if (!heldAmmo.isEmpty()) {
      ItemStack heldStack = slimeknights.tconstruct.library.utils.ItemStackNbtHelper.parse(heldAmmo);
