...
public final class ModifierUtil {
  ...
  public static int getModifierLevel(ItemStack stack, ModifierId modifier) {
    if (!stack.isEmpty() && stack.is(TinkerTags.Items.MODIFIABLE)) {
      ...
      if (!nbt.isEmpty() && nbt.contains(ToolStack.TAG_MODIFIERS)) {
        ListTag list = nbt.getList(ToolStack.TAG_MODIFIERS).orElse(new ListTag());
        int size = list.size();
        if (size > 0) {
  ...
  public static boolean hasUpgrades(ItemStack stack) {
    if (!stack.isEmpty() && stack.is(TinkerTags.Items.MODIFIABLE)) {
      CompoundTag nbt = stack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag();
      return !nbt.isEmpty() && !nbt.getList(ToolStack.TAG_UPGRADES).orElse(new ListTag()).isEmpty();
    }
    return false;
  }
  ...
  public static boolean checkVolatileFlag(ItemStack stack, Identifier flag) {
    CompoundTag nbt = stack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag();
    if (!nbt.isEmpty() && nbt.contains(ToolStack.TAG_VOLATILE_MOD_DATA)) {
      return nbt.getCompound(ToolStack.TAG_VOLATILE_MOD_DATA).orElse(new CompoundTag()).getBoolean(flag.toString()).orElse(false);
    }
    return false;
  }
  ...
  public static boolean checkPersistentPresent(ItemStack stack, Identifier key) {
    CompoundTag nbt = stack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag();
    if (!nbt.isEmpty() && nbt.contains(ToolStack.TAG_PERSISTENT_MOD_DATA)) {
      return nbt.getCompound(ToolStack.TAG_PERSISTENT_MOD_DATA).orElse(new CompoundTag()).contains(key.toString());
    }
    return false;
  }
  ...
  public static int getVolatileInt(ItemStack stack, Identifier flag) {
    CompoundTag nbt = stack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag();
    if (!nbt.isEmpty() && nbt.contains(ToolStack.TAG_VOLATILE_MOD_DATA)) {
      return nbt.getCompound(ToolStack.TAG_VOLATILE_MOD_DATA).orElse(new CompoundTag()).getInt(flag.toString()).orElse(0);
    }
    return 0;
  }
  ...
  public static int getPersistentInt(ItemStack stack, Identifier flag, int defealtValue) {
    CompoundTag nbt = stack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag();
    if (!nbt.isEmpty() && nbt.contains(ToolStack.TAG_PERSISTENT_MOD_DATA)) {
      CompoundTag persistent = nbt.getCompound(ToolStack.TAG_PERSISTENT_MOD_DATA).orElse(new CompoundTag());
      String flagString = flag.toString();
      if (persistent.contains(flagString)) {
        return persistent.getInt(flagString).orElse(0);
      }
    }
    return defealtValue;
  }
  ...
  public static String getPersistentString(ItemStack stack, Identifier flag) {
    CompoundTag nbt = stack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag();
    if (!nbt.isEmpty() && nbt.contains(ToolStack.TAG_PERSISTENT_MOD_DATA)) {
      return nbt.getCompound(ToolStack.TAG_PERSISTENT_MOD_DATA).orElse(new CompoundTag()).getString(flag.toString()).orElse("");
    }
    return "";
  }

  /** Returns a blocking animation while a chargeable tool is also configured to block. */
  public static ItemUseAnimation blockWhileCharging(IToolStackView tool, ItemUseAnimation fallback) {
    return canPerformAction(tool, net.neoforged.neoforge.common.ItemAbility.get("shield_block")) ? ItemUseAnimation.BLOCK : fallback;
  }

  /** Checks if the given tool can perform the given NeoForge tool action. */
  public static boolean canPerformAction(IToolStackView tool, ItemAbility itemAbility) {
    if (tool.isBroken()) {
      return false;
    }
    if (tool.getHook(ToolHooks.TOOL_ACTION).canPerformAction(tool, itemAbility)) {
      return true;
    }
    for (ModifierEntry entry : tool.getModifierList()) {
      if (entry.getHook(ModifierHooks.TOOL_ACTION).canPerformAction(tool, entry, itemAbility)) {
        return true;
      }
    }
    return false;
