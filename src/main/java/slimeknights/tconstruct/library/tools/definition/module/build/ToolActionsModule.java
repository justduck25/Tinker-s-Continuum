...
public record ToolActionsModule(Set<ItemAbility> actions) implements ToolActionToolHook, ToolModule {
  ...
  public static final RecordLoadable<ToolActionsModule> LOADER = RecordLoadable.create(Loadables.TOOL_ACTION.set().requiredField("tool_actions", module -> module.actions().stream().map(ItemAbility::name).collect(Collectors.toSet())), actions -> new ToolActionsModule(actions.stream().map(ItemAbility::get).collect(Collectors.toSet())));
