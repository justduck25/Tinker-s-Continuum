...
public class MaterialStatsModule implements ToolStatsHook, ToolTraitHook, ToolMaterialHook, MaterialRepairToolHook, ToolModule, VolatileDataToolHook {
  ...
  public void addVolatileData(IToolContext context, ToolDataNBT volatileData) {
    ...
    for (int i = 0; i < statTypes.size(); i++) {
      ...
      max = RarityModule.max(max, material.get().getRarity());
    }
    if (shiny) {
  ...
  public static class Builder {
    ...
    public Builder stat(MaterialStatType<?> stat, float scale) {
      return stat(new MaterialStatsId(stat.getId()), scale);
    }
  ...
  public static class ArmorBuilder implements ArmorModuleBuilder<MaterialStatsModule> {
    ...
    public ArmorBuilder stat(List<? extends MaterialStatType<?>> stats, float scale) {
      for (ArmorType slotType : slotTypes) {
        getBuilder(slotType).stat(new MaterialStatsId(stats.get(slotType.ordinal()).getId()), scale);
      }
      return this;
    }
