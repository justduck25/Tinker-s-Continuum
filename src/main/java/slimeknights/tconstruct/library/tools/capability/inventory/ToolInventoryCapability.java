...
public class ToolInventoryCapability extends InventoryModifierHookIterator<ModifierEntry> implements IItemHandlerModifiable {
  ...
  /** Stack to update after mutating the tool NBT. Null for read-only or detached handlers. */
  @Nullable
  ...
  public static boolean isBlacklisted(ItemStack stack) {
    return !stack.getItem().canFitInsideContainerItems() || stack.is(TinkerTags.Items.TOOL_INVENTORY_BLACKLIST) || false;
  }
  ...
  /** Flushes mutated tool data back to the ItemStack. */
  private void updateStack() {
    if (stack != null && tool.get() instanceof ToolStack toolStack) {
  ...
  public ItemStack insertItem(int slot, ItemStack stack, boolean simulate) {
    ...
    if (current.isEmpty()) {
      ...
      if (!simulate) {
        setAndCache(inventory, localSlot, slot, copyStackWithSize(stack, canInsert));
      }
    } else {
    ...
    return copyStackWithSize(stack, leftover);
  }
  ...
  public ItemStack extractItem(int slot, int amount, boolean simulate) {
    ...
    ItemStack result = copyStackWithSize(current, amount);
    if (!simulate) {
      if (amount == current.getCount()) {
  ...
  public static class Provider implements IToolCapabilityProvider {
    ...
    public Provider(ItemStack stack, Supplier<? extends IToolStackView> tool) {
      handler = new ToolInventoryCapability(tool, stack);
    }

    @Override
    public Object getCapability(IToolStackView tool, Object cap) {
      if (cap == Capabilities.Item.ITEM && tool.getVolatileData().getInt(TOTAL_SLOTS) > 0) {
        return handler;
      }
      return null;
    }
  ...
  public static InteractionResult tryOpenContainer(ItemStack stack, @Nullable IToolStackView tool, ToolDefinition definition, Player player, int slotIndex) {
    IToolStackView toolView = tool != null ? tool : ToolStack.from(stack);
    IItemHandler handler = toolView.getVolatileData().getInt(TOTAL_SLOTS) > 0 ? new ToolInventoryCapability(() -> toolView, stack) : EmptyItemHandler.INSTANCE;
    int tankCapacity = ToolTankHelper.TANK_HELPER.getCapacity(toolView);
    // open if we have any slots, crafting table, or a fluid tank
    if (handler.getSlots() > 0 || tankCapacity > 0 || ModifierUtil.checkVolatileFlag(stack, CRAFTING_TABLE) || ModifierUtil.checkVolatileFlag(stack, INVENTORY_CRAFTING)) {
      if (player instanceof ServerPlayer serverPlayer) {
        serverPlayer.openMenu( new SimpleMenuProvider(
          ...
          ToolSyncType syncType = tankCapacity > 0 ? ToolSyncType.FULL_STACK : Config.COMMON.toolInventorySync.get();
