...
public interface BlockItemProviderCapability {
  ...
  static void register() {
    ModLoadingContext.get().getActiveContainer().getEventBus().addListener(RegisterCapabilitiesEvent.class, BlockItemProviderCapability::register);
  }

  /** Registers the capability with the event bus */
  private static void register(RegisterCapabilitiesEvent event) {
    event.registerItem(CAPABILITY, (stack, ignored) -> stack.getItem() instanceof BlockItem ? SimpleBlockItem.INSTANCE : null, BuiltInRegistries.ITEM.stream().toArray(ItemLike[]::new));
  }
