...
public class ToolFluidCapability extends FluidModifierHookIterator<ModifierEntry> implements IFluidHandlerItem {
  ...
  public static class Provider implements IToolCapabilityProvider {
    ...
    public Object getCapability(IToolStackView tool, Object cap) {
      if (cap == Capabilities.Fluid.ITEM && tool.getVolatileData().getInt(TOTAL_TANKS) > 0) {
        return fluidCap;
      }
      return null;
    }
  ...
  /** NeoForge transfer API wrapper for tool tanks. */
  public static class ToolItemFluidHandler extends ItemAccessResourceHandler<FluidResource> {
    public ToolItemFluidHandler(ItemAccess itemAccess) {
      super(itemAccess, 1);
    }

    private static ToolStack getTool(ItemResource accessResource) {
      return ToolStack.from(accessResource.toStack());
    }

    @Override
    protected FluidResource getResourceFrom(ItemResource accessResource, int index) {
      Objects.checkIndex(index, size());
      FluidStack fluid = ToolTankHelper.TANK_HELPER.getFluid(getTool(accessResource));
    ...
    protected int getAmountFrom(ItemResource accessResource, int index) {
      Objects.checkIndex(index, size());
      return ToolTankHelper.TANK_HELPER.getFluid(getTool(accessResource)).getAmount();
    }
    ...
    protected ItemResource update(ItemResource accessResource, int index, FluidResource newResource, int newAmount) {
      ...
      ToolTankHelper.TANK_HELPER.setFluid(tool, newAmount == 0 ? FluidStack.EMPTY : newResource.toStack(newAmount));
    ...
    protected int getCapacity(int index, FluidResource resource) {
      Objects.checkIndex(index, size());
      return ToolTankHelper.TANK_HELPER.getCapacity(getTool(itemAccess.getResource()));
    }
  }
}
