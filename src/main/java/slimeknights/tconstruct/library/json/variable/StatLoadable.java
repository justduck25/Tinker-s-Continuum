...
public enum StatLoadable implements Loadable<Stat<?>> {
  ...
  private static <T> Stat<T> parseStat(StatType<T> statType, JsonElement element, String key, TypedMap context) {
    ...
    throw new JsonSyntaxException("Unable to parse " + key + " as registry " + registry.key().registry() + " does not contain ID " + name);
  }
  ...
  private static <T> JsonElement serializeGeneric(Stat<T> stat) {
    ...
    if (location == null) {
      throw new RuntimeException("Registry " + registry.key().registry() + " does not contain object " + value);
    }
  ...
  private static <T> T decodeRegistry(FriendlyByteBuf buffer, Registry<T> registry) {
    ...
    throw new DecoderException("Unknown " + registry.key().registry() + " id " + id);
  }
  ...
  public static Component statName(Stat<?> stat) {
    ...
    if (registry == BuiltInRegistries.BLOCK) {
      ...
      name = Component.translatable(((Item) value).getDescriptionId());
