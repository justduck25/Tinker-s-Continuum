...
// TIER_STUB: import net.minecraft.world.item.Tier;
...
public class TinkerLoadables {
  ...
  /** Supplier for the HolderLookup.Provider, set during datagen. */
  public static Supplier<HolderLookup.Provider> LOOKUP_PROVIDER = () -> null;
  /** Enchantment loadable that falls back to lookupProvider when BuiltInRegistries doesn't have the enchantment registry. */
  public static final StringLoadable<Enchantment> ENCHANTMENT = TinkerEnchantmentLoadable.INSTANCE;
  ...
  /** Tier loadable from the vanilla tool material constants. */
  public static final StringLoadable<ToolMaterial> TIER = Loadables.RESOURCE_LOCATION.xmap((id, error) -> {
    ...
    return tier;
  }, (tier, error) -> HarvestTiers.getName(tier));
