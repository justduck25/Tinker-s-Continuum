...
public class ToolPartLootEntry extends LootPoolSingletonContainer {
  ...
  public static final MapCodec<ToolPartLootEntry> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
    ...
    RANDOM_MATERIAL_CODEC.fieldOf("material").forGetter(entry -> entry.material)
  ...
  protected void createItemStack(Consumer<ItemStack> consumer, LootContext context) {
    List<IToolPart> options = StreamSupport.stream(BuiltInRegistries.ITEM.getTagOrEmpty(tag).spliterator(), false)
      .map(Holder::value)
      .flatMap(item -> item instanceof IToolPart part ? java.util.stream.Stream.of(part) : java.util.stream.Stream.empty())
      .toList();
    if (!options.isEmpty()) {
      ...
      if (choice.canUseMaterial(material.getMaterialId())) {
