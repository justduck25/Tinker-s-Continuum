...
public class MobEquipmentManager extends SimpleJsonResourceReloadListener<JsonElement> {
  ...
  public static void init() {
    ...
    NeoForge.EVENT_BUS.addListener(EventPriority.NORMAL, false, EntityJoinLevelEvent.class, INSTANCE::onEntityJoinLevel);
  }
  ...
  private void onEntityJoinLevel(EntityJoinLevelEvent event) {
    if (event.loadedFromDisk() || event.getLevel().isClientSide() || !(event.getEntity() instanceof Mob mob)) {
      return;
    }
    List<MobEquipment> equipment = get(mob.getType());
    if (!equipment.isEmpty()) {
      MobEquipment.apply(equipment, mob, event);
    }
  }
}
