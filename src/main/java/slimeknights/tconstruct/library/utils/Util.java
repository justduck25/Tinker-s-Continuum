...
public class Util {
  ...
  public static String makeTranslationKey(String base, @Nullable Identifier name) {
    return name == null ? base + ".unregistered_sadface" : base + "." + name.getNamespace() + "." + name.getPath().replace('/', '.');
  }
  ...
  public static Direction directionFromOffset(BlockPos pos, BlockPos neighbor) {
    ...
    for (Direction direction : Direction.values()) {
      if (direction.getStepX() == offset.getX() && direction.getStepY() == offset.getY() && direction.getStepZ() == offset.getZ()) {
        return direction;
      }
    }
