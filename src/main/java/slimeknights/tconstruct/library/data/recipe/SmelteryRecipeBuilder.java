...
public class SmelteryRecipeBuilder {
  ...
  /** Creates an Ingredient from a TagKey */
  ...
  private void itemMelting(float scale, String output, float factor, Identifier itemName, boolean damagable) {
    ...
    builder.save(consumer, location(meltingFolder, output));
  }
  ...
  private void oreMelting(float scale, String tagPrefix, @Nullable TagKey<Item> size, float factor, String output, boolean forceOptional) {
    ...
    Ingredient baseIngredient = tagIngredient(itemTag(tagName));
    ...
    if (size == Tags.Items.ORE_RATES_SINGULAR) {
      ingredient = DifferenceIngredient.of(baseIngredient, tagIngredient(TinkerTags.Items.NON_SINGULAR_ORE_RATES));
      ...
      ingredient = IntersectionIngredient.of(baseIngredient, tagIngredient(size));
    ...
    if (oreByproducts.length == 0) {
      ...
      for (IByproduct byproduct : oreByproducts) {
        ...
        supplier.get()
                ...
                .save(byproductOutput, location.withSuffix("_byproduct_" + index));
        index++;
        if (alwaysPresent) {
          break;
        }
  ...
  public SmelteryRecipeBuilder meltingCasting(float scale, String tagPrefix, String castMaterial, float factor, boolean forceOptional) {
    return meltingCasting(scale, tagPrefix, tagIngredient(itemTag(tagPrefix + "s/" + castMaterial)), factor, forceOptional);
  }
  ...
  public SmelteryRecipeBuilder metal() {
    ...
    blockCasting(9, slimeknights.mantle.recipe.ingredient.EmptyIngredient.VANILLA, false);
  ...
  public SmelteryRecipeBuilder gem(int storageSize) {
    ...
    blockCasting(storageSize, slimeknights.mantle.recipe.ingredient.EmptyIngredient.VANILLA, false);
