...
public abstract class AbstractToolItemModelProvider extends GenericDataProvider {
  ...
  public CompletableFuture<?> run(CachedOutput cache) {
    ...
    Stream<CompletableFuture<?>> generatedModels = models.entrySet().stream().flatMap(entry -> {
      Identifier id = resource(entry.getKey());
      JsonObject model = entry.getValue();
      return Stream.of(
        saveJson(cache, id, model, null),
        DataProvider.saveStable(cache, itemDefinition(id, model), itemDefinitions.json(id)));
    });
    Stream<CompletableFuture<?>> rootDefinitions = rootModels.entrySet().stream().map(entry ->
      DataProvider.saveStable(cache, toolItemDefinition(entry.getKey(), entry.getValue(), models, rootModels), itemDefinitions.json(entry.getKey())));
    return allOf(Stream.concat(generatedModels, rootDefinitions));
  }
  ...
  private static JsonObject toolItemDefinition(Identifier item, JsonObject sourceModel, Map<String,JsonObject> generatedModels, Map<Identifier,JsonObject> rootModels) {
    JsonObject definition = new JsonObject();
    JsonObject model = itemModel(item, sourceModel);
  ...
  private static JsonObject overrideModel(String modelName, Map<String,JsonObject> generatedModels, Map<Identifier,JsonObject> rootModels) {
