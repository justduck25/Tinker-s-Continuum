...
public abstract class AbstractToolItemModelProvider extends GenericDataProvider {
  ...
  protected final Map<Identifier,JsonObject> rootModels = new HashMap<>();
  ...
  public CompletableFuture<?> run(CachedOutput cache) {
    try {
      ...
      Stream.Builder<CompletableFuture<?>> futures = Stream.builder();
      for (Map.Entry<String,JsonObject> entry : models.entrySet()) {
        Identifier id = resource(entry.getKey());
        JsonObject model = entry.getValue();
        futures.add(saveJson(cache, id, model, null));
        futures.add(DataProvider.saveStable(cache, itemDefinition(id, model), itemDefinitions.json(id)));
      }
      for (Map.Entry<Identifier,JsonObject> entry : rootModels.entrySet()) {
        futures.add(DataProvider.saveStable(cache, toolItemDefinition(entry.getKey(), entry.getValue(), models, rootModels), itemDefinitions.json(entry.getKey())));
      }
      return allOf(futures.build());
    } catch (IOException e) {
      return CompletableFuture.failedFuture(e);
    }
  }

  private static JsonObject itemDefinition(Identifier item, JsonObject sourceModel) {
    JsonObject definition = new JsonObject();
    JsonObject model = itemModel(item, sourceModel);
    definition.add("model", model);
    return definition;
  }

  private JsonObject toolItemDefinition(Identifier item, JsonObject sourceModel, Map<String,JsonObject> generatedModels, Map<Identifier,JsonObject> rootModels) throws IOException {
    JsonObject definition = new JsonObject();
    JsonObject model = itemModel(item, sourceModel);
    if (sourceModel.has("overrides")) {
      JsonArray overrides = sourceModel.getAsJsonArray("overrides");
      for (int i = 0; i < overrides.size(); i++) {
        JsonObject override = overrides.get(i).getAsJsonObject();
        JsonObject predicate = GsonHelper.getAsJsonObject(override, "predicate");
        JsonObject selected = overrideModel(GsonHelper.getAsString(override, "model"), generatedModels, rootModels);
        model = predicateModel(predicate, selected, model);
      }
    }
    definition.add("model", model);
    return definition;
  ...
  private static JsonObject predicateModel(JsonObject predicate, JsonObject selected, JsonObject fallback) {
    JsonObject model = selected;
    for (Map.Entry<String,JsonElement> entry : predicate.entrySet()) {
      String property = entry.getKey();
      float threshold = entry.getValue().getAsFloat();
      if ("tconstruct:broken".equals(property)) {
        JsonObject condition = new JsonObject();
        condition.addProperty("type", "minecraft:condition");
        condition.addProperty("property", property);
        condition.add("on_true", model);
        condition.add("on_false", fallback.deepCopy());
        model = condition;
      } else {
        JsonObject range = new JsonObject();
        range.addProperty("type", "minecraft:range_dispatch");
        range.addProperty("property", property);
        JsonArray entries = new JsonArray();
        JsonObject rangeEntry = new JsonObject();
        rangeEntry.addProperty("threshold", threshold);
        rangeEntry.add("model", model);
        entries.add(rangeEntry);
        range.add("entries", entries);
        range.add("fallback", fallback.deepCopy());
        model = range;
      }
    }
    return model;
  ...
  private JsonObject overrideModel(String modelName, Map<String,JsonObject> generatedModels, Map<Identifier,JsonObject> rootModels) throws IOException {
    Identifier modelId = Identifier.parse(modelName);
    String path = modelId.getPath();
    if (path.startsWith("item/")) {
      path = path.substring("item/".length());
    }
    JsonObject generated = generatedModels.get(path);
    if (generated != null) {
      JsonObject source = resolveToolSource(generated, generatedModels, rootModels);
      return itemModel(Identifier.fromNamespaceAndPath(modelId.getNamespace(), path), source);
    }
    // Item overrides may refer to a source model that is not generated by this provider.
    // Load it so parent chains such as javelin -> item/javelin still resolve to the tool loader.
    try {
      JsonObject source = readJson(Identifier.fromNamespaceAndPath(modelId.getNamespace(), path));
      source = resolveToolSource(source, generatedModels, rootModels);
      return itemModel(Identifier.fromNamespaceAndPath(modelId.getNamespace(), path), source);
  ...
  private static JsonObject resolveToolSource(JsonObject model, Map<String,JsonObject> generatedModels, Map<Identifier,JsonObject> rootModels) {
    if (model.has("loader") && "tconstruct:tool".equals(model.get("loader").getAsString())) {
      return model;
    }
    if (model.has("parent")) {
      Identifier parent = Identifier.parse(GsonHelper.getAsString(model, "parent"));
      String parentPath = parent.getPath();
      if (parentPath.startsWith("item/")) {
        parentPath = parentPath.substring("item/".length());
      }
      JsonObject generatedParent = generatedModels.get(parentPath);
      if (generatedParent != null) {
        return resolveToolSource(generatedParent, generatedModels, rootModels);
      }
      JsonObject rootParent = rootModels.get(Identifier.fromNamespaceAndPath(parent.getNamespace(), parentPath));
      if (rootParent != null) {
        return rootParent;
      }
    }
  ...
  private static JsonObject itemModel(Identifier item, JsonObject sourceModel) {
    JsonObject model = new JsonObject();
    if (sourceModel.has("loader") && "tconstruct:tool".equals(sourceModel.get("loader").getAsString())) {
      model.addProperty("type", "tconstruct:tool");
      model.addProperty("model", item.getNamespace() + ":item/" + item.getPath());
      copy(sourceModel, model, "textures");
      copy(sourceModel, model, "parts");
      copy(sourceModel, model, "modifier_maps");
      copy(sourceModel, model, "first_modifiers");
      copy(sourceModel, model, "show_traits");
      copy(sourceModel, model, "large");
      copy(sourceModel, model, "large_offset");
      copy(sourceModel, model, "modifier_roots");
      copy(sourceModel, model, "ammo");
    } else {
      model.addProperty("type", "minecraft:model");
      model.addProperty("model", item.getNamespace() + ":item/" + item.getPath());
    }
  ...
  protected JsonObject readJson(Identifier path) throws IOException {
    try (BufferedReader reader = resourceManager.getResource(path.withPrefix("models/item/").withSuffix(".json")).orElseThrow(() -> new FileNotFoundException("Model " + path + " not found")).openAsReader()) {
      JsonObject model = GsonHelper.parse(reader);
      if (model.has("loader") && "tconstruct:tool".equals(model.get("loader").getAsString())) {
        rootModels.putIfAbsent(path, model.deepCopy());
      }
      return model;
    }
  }
  ...
  protected void withDisplay(String destination, Identifier parent, JsonObject properties) {
    ...
    // Add particle texture from the parent to avoid missing texture warning in NeoForge 26.1
    JsonObject textures = new JsonObject();
    textures.addProperty("layer0", "tconstruct:item/empty");
    textures.addProperty("particle", "tconstruct:item/empty");
