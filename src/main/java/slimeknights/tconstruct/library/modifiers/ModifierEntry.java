...
public class ModifierEntry implements Comparable<ModifierEntry> {
  ...
  public static final RecordLoadable<ModifierEntry> LOADABLE = RecordLoadable.create(
    ...
    IntLoadable.FROM_ONE.defaultField(TAG_LEVEL, 1, true, e -> Integer.valueOf(e.getLevel())),
    (id, level) -> new ModifierEntry(id, level));
  ...
  /** Creates a copy with a new level (replaces Lombok @With) */
  public ModifierEntry withLevel(int newLevel) {
    return new ModifierEntry(this.modifier, newLevel);
  }

  /** Gets the level (replaces Lombok @Getter) */
  public int getLevel() {
    return level;
  }
  ...
  public ModifierEntry addAmount(int amount, int needed) {
    ...
    // static constructor automatically handles cases of needed being 0/amount being full (non-incremental level raise) and amount being 0 (ignore level raise)
  ...
  public static ModifierEntry readFromNBT(CompoundTag tag) {
    if (tag.contains(TAG_MODIFIER)) {
      ModifierId id = ModifierId.tryParse(tag.getString(TAG_MODIFIER).orElse(""));
      int level = tag.getInt(TAG_LEVEL).orElse(0);
      if (id != null && level > 0) {
        // incremental just has more tags, if they are missing they will just 0 and of will give us the base class
        return IncrementalModifierEntry.of(id, level, tag.getInt(TAG_AMOUNT).orElse(0), tag.getInt(TAG_NEEDED).orElse(0));
      }
    }
    return EMPTY;
  }
