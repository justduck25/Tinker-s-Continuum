...
public class ModifierMaxLevel {
  ...
  public void set(EquipmentSlot slot, float level) {
    float oldLevel = levels[slimeknights.tconstruct.library.tools.context.EquipmentContext.slotIndex(slot)];
    if (level != oldLevel) {
      // first, update level
      levels[slimeknights.tconstruct.library.tools.context.EquipmentContext.slotIndex(slot)] = level;
      ...
      if (level >= max) {
        ...
        for (EquipmentSlot armorSlot : ModifiableArmorMaterial.ARMOR_SLOTS) {
          float value = levels[slimeknights.tconstruct.library.tools.context.EquipmentContext.slotIndex(armorSlot)];
          if (value > max) {
            max = value;
            maxSlot = armorSlot;
          }
