  ...
  public static class SlotInCharge {
    ...
    private void removeSlot(EquipmentSlot slotType) {
      ...
      for (EquipmentSlot armorSlot : ModifiableArmorMaterial.ARMOR_SLOTS) {
        if (levels[slimeknights.tconstruct.library.tools.context.EquipmentContext.slotIndex(armorSlot)] > 0) {
          inCharge = armorSlot;
          return;
        }
      }
      ...
      for (EquipmentSlot hand : InteractionHandler.HAND_SLOTS) {
        if (levels[slimeknights.tconstruct.library.tools.context.EquipmentContext.slotIndex(hand)] > 0) {
          inCharge = hand;
          return;
        }
      }
