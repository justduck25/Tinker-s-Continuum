...
public record ToolActionTransformModule(ItemAbility action, SoundEvent sound, boolean requireGround, int eventId, ModifierCondition<IToolStackView> condition) implements BlockTransformModule, ToolActionModifierHook, ConditionalModule<IToolStackView> {
  ...
  public static final RecordLoadable<ToolActionTransformModule> LOADER = RecordLoadable.create(
    Loadables.TOOL_ACTION.requiredField("tool_action", module -> module.action().name()),
    ...
    (action, sound, requireGround, eventId, condition) -> new ToolActionTransformModule(ItemAbility.get(action), sound, requireGround, eventId, condition));
  ...
  public boolean canPerformAction(IToolStackView tool, ModifierEntry modifier, ItemAbility itemAbility) {
    return condition.matches(tool, modifier) && this.action == itemAbility;
  }
