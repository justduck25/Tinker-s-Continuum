  ...
  public boolean onProjectileHitEntity(ModifierNBT modifiers, ModDataNBT persistentData, ModifierEntry modifier, Projectile projectile, EntityHitResult hit, @Nullable LivingEntity attacker, @Nullable LivingEntity target) {
    ...
    if (modifierLevel.test(modifier.getLevel()) && !persistentData.getBoolean(key)) {
      ...
      if (TinkerPredicate.matches(this.target, target) && TinkerPredicate.matches(this.holder, attacker)) {
        float multiplier = 1;
        if (persistentData.contains(AMMO_MULTIPLIER, Tag.TAG_FLOAT)) {
          multiplier *= persistentData.getFloat(AMMO_MULTIPLIER);
        }
        if (persistentData.contains(BOW_MULTIPLIER, Tag.TAG_FLOAT)) {
          multiplier *= persistentData.getFloat(BOW_MULTIPLIER);
        }
        if (projectile instanceof AbstractArrow arrow) {
          arrow.setBaseDamage(formula.apply(modifiers, persistentData, modifier, projectile, hit, attacker, target, ProjectileWithPower.getDamage(projectile), multiplier));
