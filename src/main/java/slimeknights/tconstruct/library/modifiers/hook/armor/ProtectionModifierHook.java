...
public interface ProtectionModifierHook {
  ...
  static double getProtectionCap(LivingEntity living, TinkerDataCapability.Holder capability) {
    float dataCap = capability == null ? 0f : capability.get(TinkerDataKeys.PROTECTION_CAP, 0f);
    return Math.min(living.getAttributeValue(TinkerAttributes.PROTECTION_CAP) * 25f + dataCap, 25 * 0.95f);
  }
