...
public interface ModifierLevelDisplay extends IHaveLoader {
  ...
  record UniqueForLevels(int unique, boolean firstUnique) implements ModifierLevelDisplay {
    ...
    public Component nameForLevel(Modifier modifier, int level) {
      ...
      if (level <= unique) {
        String key = modifier.getTranslationKey() + "." + level;
        MutableComponent name = Component.translatable(key);
        TextColor color = ResourceColorManager.getOrNull(key);
        return color != null ? name.withStyle(style -> style.withColor(color)) : modifier.applyStyle(name);
      }
      return DEFAULT.nameForLevel(modifier, level);
    }
  ...
  /** Displays the modifier name using a remapped level. */
