...
public class ModifierSetWorktableRecipeBuilder extends AbstractSizedIngredientRecipeBuilder<ModifierSetWorktableRecipeBuilder> {
  ...
  public ModifierSetWorktableRecipeBuilder setTools(TagKey<Item> tag) {
    return this.setTools(slimeknights.tconstruct.library.recipe.ingredient.LegacyIngredientType.ofTag(tag));
  }
  ...
  public void save(RecipeOutput consumer, Identifier id) {
    ...
    if (tools == slimeknights.mantle.recipe.ingredient.EmptyIngredient.VANILLA) {
      throw new IllegalStateException("Tools cannot be empty");
    }
    var key = recipeKey(id);
    consumer.accept(key, new ModifierSetWorktableRecipe(id, dataKey, inputs, tools, modifierPredicate, addToSet, allowTraits), buildOptionalAdvancement(key, "modifiers"));
  }
}
