...
public class MaterialRecipeBuilder extends AbstractRecipeBuilder<MaterialRecipeBuilder> {
  ...
  private Ingredient ingredient = slimeknights.tconstruct.library.recipe.casting.AbstractCastingRecipe.EMPTY_INGREDIENT;
  @Setter
  private int value = 1;
  @Setter
  ...
  public MaterialRecipeBuilder setIngredient(TagKey<Item> tag) {
    return this.setIngredient(slimeknights.tconstruct.library.recipe.ingredient.LegacyIngredientType.ofTag(tag));
  }
  ...
  public void save(RecipeOutput consumerIn, Identifier id) {
    ...
    if (this.ingredient == slimeknights.tconstruct.library.recipe.casting.AbstractCastingRecipe.EMPTY_INGREDIENT) {
    ...
    consumerIn.accept(key, new MaterialRecipe(id, group, ingredient, value, needed, material, leftover), advancement);
  }
}
