...
public class IncrementalModifierRecipeBuilder extends AbstractModifierRecipeBuilder<IncrementalModifierRecipeBuilder> {
  private Ingredient input = slimeknights.mantle.recipe.ingredient.EmptyIngredient.VANILLA;
  ...
  public IncrementalModifierRecipeBuilder setInput(TagKey<Item> tag, int amountPerItem, int neededPerLevel) {
    return setInput(tagIngredient(tag), amountPerItem, neededPerLevel);
  }
  ...
  public void save(RecipeOutput consumer, Identifier id) {
    if (input == slimeknights.mantle.recipe.ingredient.EmptyIngredient.VANILLA) {
      throw new IllegalStateException("Must set input");
    }
    var key = recipeKey(id);
    consumer.accept(key, new IncrementalModifierRecipe(id, input, amountPerItem, neededPerLevel, tools, maxToolSize, result, ModifierEntry.VALID_LEVEL.range(minLevel, maxLevel), slots, leftover, allowCrystal, checkTraitLevel), buildOptionalAdvancement(key, "modifiers"));
  }
}
