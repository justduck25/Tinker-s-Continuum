...
public class IncrementalModifierRecipe extends AbstractModifierRecipe {
  ...
  protected List<List<ItemStack>> getInputs() {
    return getInputs(ContextMap.EMPTY);
  }

  /** Gets the list of input stacks for display using a recipe display context. */
  protected List<List<ItemStack>> getInputs(ContextMap displayContext) {
    ImmutableList.Builder<List<ItemStack>> builder = ImmutableList.builder();

    // fill extra item slots
    List<ItemStack> items = getDisplayStacks(input, 1, displayContext);
    ...
    List<ItemStack> fullSize = items.stream().map(stack -> stack.copyWithCount(maxStackSize)).collect(Collectors.toList());
    while (needed > maxStackSize) {
      builder.add(fullSize);
      needed -= maxStackSize;
    }
  ...
  @Override
  public int getInputCount(ContextMap displayContext) {
    return getInputs(displayContext).size();
  }
  ...
  public List<ItemStack> getDisplayItems(int slot, ContextMap displayContext) {
    List<List<ItemStack>> inputs = getInputs(displayContext);
  ...
  /** @deprecated use {@link #getDisplayItems(int, ContextMap)} */
  @Deprecated
  protected List<ItemStack> getDisplayItemsOld(int slot) {
