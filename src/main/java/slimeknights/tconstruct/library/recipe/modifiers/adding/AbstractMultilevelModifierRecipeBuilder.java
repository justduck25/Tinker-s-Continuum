...
public abstract class AbstractMultilevelModifierRecipeBuilder<T extends AbstractMultilevelModifierRecipeBuilder<T>> extends AbstractRecipeBuilder<T> {
  ...
  protected Ingredient tools = slimeknights.mantle.recipe.ingredient.EmptyIngredient.VANILLA;
  ...
  public T setTools(TagKey<Item> tag) {
    return this.setTools(tagIngredient(tag));
  }

  protected static Ingredient tagIngredient(TagKey<Item> tag) {
  ...
  public T saveSalvage(RecipeOutput consumer, Identifier id) {
    ...
    for (LevelEntry levelEntry : levels) {
      if (levelEntry.slots() != null) {
        var key = recipeKey(id.withSuffix("_level_" + levelEntry.level().min()));
        consumer.accept(key, new ModifierSalvage(key.identifier(), tools, maxToolSize, result, levelEntry.level(), levelEntry.slots()), null);
      }
    }
    return self();
  }
