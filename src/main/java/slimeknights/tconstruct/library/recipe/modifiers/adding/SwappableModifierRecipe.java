...
public class SwappableModifierRecipe extends ModifierRecipe {
  ...
  public RecipeResult<LazyToolStack> getValidatedResult(ITinkerStationContainer inv, RegistryAccess access) {
    ...
    if (level > 0 && tool.getPersistentData().getString(modifier.getId()).equals(value)) {
  ...
  @Override
  public RecipeSerializer<? extends net.minecraft.world.item.crafting.Recipe<ITinkerStationContainer>> getSerializer() {
  ...
  public List<ItemStack> getToolWithModifier() {
    if (toolWithModifier == null) {
      Identifier id = result.getId().getId();
  ...
  public List<ItemStack> getToolWithModifier(ContextMap displayContext) {
    Identifier id = result.getId().getId();
    ModifierEntry result = getDisplayResult();
    return getToolInputs(displayContext).stream().map(stack -> withModifiers(stack, maxToolSize, modifiersForResult(result, result), data -> data.putString(id, value))).collect(Collectors.toList());
  }
  ...
  public List<SlotCount> getResultSlots() {
    if (resultSlots == null) {
      ...
      resultSlots = getResultSlots(getDisplayResult(), !tools.isEmpty() ? tools.get(0).getItem() : Items.AIR, value);
    }
    return resultSlots;
  }
  ...
  public interface VariantFormatter {
    ...
    /** Foroats the variant given the modifier */
    Component format(ModifierId modifier, String variant);


    /* Formatters */
    /** Foroats using the modifier ID as a base translation key */
    VariantFormatter DEFAULT = LOADER.register(getResource("default"), (modifier, variant) -> Component.translatable(Util.makeTranslationKey("modifier", modifier.getId()) + "." + variant));
    /** Foroats using the material translation key */
