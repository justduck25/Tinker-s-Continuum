...
public abstract class AbstractModifierRecipe implements ITinkerStationRecipe, IDisplayModifierRecipe {
  ...
  @Override
  public List<ItemStack> getToolWithoutModifier(ContextMap displayContext) {
    int min = level.min() - 1;
    ModifierEntry existing = min > 0 ? new ModifierEntry(result, min) : null;
    ModifierEntry displayResult = getDisplayResult();
    return getToolInputs(displayContext).stream().map(stack -> withModifiers(stack, maxToolSize, modifiersForResult(displayResult, existing))).collect(Collectors.toList());
  }
  ...
  @Override
  public List<ItemStack> getToolWithModifier(ContextMap displayContext) {
    ModifierEntry result = getDisplayResult();
    return getToolInputs(displayContext).stream().map(stack -> withModifiers(stack, maxToolSize, modifiersForResult(result, result))).collect(Collectors.toList());
  }
  ...
  public static List<SlotCount> getResultSlots(ModifierEntry result, Item tool, String variant) {
    ...
    if (!variant.isEmpty()) {
      persistentData.putString(result.getId().getId(), variant);
    }
  ...
  public List<SlotCount> getResultSlots() {
    if (resultSlots == null) {
      ...
      List<ItemStack> tools = getToolInputs();
      resultSlots = getResultSlots(getDisplayResult(), !tools.isEmpty() ? tools.get(0).getItem() : Items.AIR, "");
    }
    return resultSlots;
  }

  /** Resolves representative tool stacks for JEI/book displays without dereferencing tags during recipe construction. */
  protected static List<ItemStack> getToolInputStacks(Ingredient ingredient) {
    return getToolInputStacks(ingredient, ContextMap.EMPTY);
  }

  /** Resolves representative tool stacks for JEI/book displays using the given recipe display context. */
  protected static List<ItemStack> getToolInputStacks(Ingredient ingredient, ContextMap displayContext) {
    return getDisplayStacks(ingredient, 1, displayContext);
  }

  /** Resolves item stacks for book/JEI displays without dereferencing tag placeholders too early. */
  protected static List<ItemStack> getDisplayStacks(Ingredient ingredient, int amount, ContextMap displayContext) {
    SlotDisplay display = null;
    List<ItemStack> stacks;
    try {
      display = ingredient.display();
      stacks = display.resolveForStacks(displayContext).stream()
    ...
    if (stacks.isEmpty() && display != null) {
      stacks = resolveDisplayFallback(display, amount);
    }
    if (stacks.isEmpty()) {
      try {
        stacks = ingredient.items()
          .map(holder -> new ItemStack(holder.value(), amount))
          .filter(stack -> !stack.isEmpty())
          .toList();
      } catch (UnsupportedOperationException | IllegalStateException e) {
        stacks = List.of();
    ...
    if (stacks.isEmpty()) {
      stacks = resolveByTestingItems(ingredient, amount);
    }
    return stacks;
  }

  /** Fallback resolver for nested SlotDisplay wrappers when a context is unavailable. */
  private static List<ItemStack> resolveDisplayFallback(SlotDisplay display, int amount) {
    if (display instanceof SlotDisplay.TagSlotDisplay tagDisplay) {
      ...
      return List.copyOf(tagStacks);
    }
    if (display instanceof SlotDisplay.Composite composite) {
      return composite.contents().stream().flatMap(entry -> resolveDisplayFallback(entry, amount).stream()).toList();
    }
    if (display instanceof SlotDisplay.WithRemainder remainder) {
      return resolveDisplayFallback(remainder.input(), amount);
    }
    if (display instanceof SlotDisplay.OnlyWithComponent component) {
      return resolveDisplayFallback(component.source(), amount);
    }
    return List.of();
  }

  /** Last resort display resolver for NeoForge custom ingredients that expose neither SlotDisplay nor item holders. */
  private static List<ItemStack> resolveByTestingItems(Ingredient ingredient, int amount) {
