...
public class OverslimeModifierRecipe implements ITinkerStationRecipe, IDisplayModifierRecipe {
  ...
  public static final RecordLoadable<OverslimeModifierRecipe> LOADER = RecordLoadable.create(
    ...
    IngredientLoadable.DISALLOW_EMPTY.defaultField("tools", Ingredient.of(net.minecraft.world.item.Items.STONE), true, r -> r.tools),
  ...
  public OverslimeModifierRecipe(Identifier id, Ingredient ingredient, int restoreAmount) {
    this(id, slimeknights.tconstruct.library.recipe.ingredient.LegacyIngredientType.ofTag(TinkerTags.Items.DURABILITY), ingredient, restoreAmount);
  }
  ...
  public List<ItemStack> getDisplayItems(int slot) {
    return getDisplayItems(slot, ContextMap.EMPTY);
  }

  @Override
  public List<ItemStack> getDisplayItems(int slot, ContextMap displayContext) {
    if (slot == 0) {
      return AbstractModifierRecipe.getDisplayStacks(ingredient, 1, displayContext);
    }
  ...
  public List<ItemStack> getToolWithoutModifier() {
    if (toolWithoutModifier == null) {
      toolWithoutModifier = getToolWithoutModifier(ContextMap.EMPTY);
    }
  ...
  public List<ItemStack> getToolWithoutModifier(ContextMap displayContext) {
    return AbstractModifierRecipe.getToolInputStacks(this.tools, displayContext).stream().map(MAP_TOOL_STACK_FOR_RENDERING).toList();
  }

  @Override
  public List<ItemStack> getToolWithModifier() {
    if (toolWithModifier == null) {
      toolWithModifier = getToolWithModifier(ContextMap.EMPTY);
    }
  ...
  public List<ItemStack> getToolWithModifier(ContextMap displayContext) {
    ...
    return AbstractModifierRecipe.getToolInputStacks(this.tools, displayContext).stream()
