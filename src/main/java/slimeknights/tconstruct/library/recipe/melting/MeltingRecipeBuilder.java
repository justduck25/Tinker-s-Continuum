...
public class MeltingRecipeBuilder extends AbstractRecipeBuilder<MeltingRecipeBuilder> {
  ...
  public static MeltingRecipeBuilder melting(Ingredient input, Fluid fluid, int amount, float timeFactor) {
    int temperature = getTemperature(fluid);
    return melting(input, FluidOutput.fromFluid(fluid, amount), temperature, IMeltingRecipe.calcTime(temperature, timeFactor));
  }
  ...
  /**
   * Adds a byproduct to this recipe
   * @param fluid   Byproduct fluid
   * @param amount  Byproduct amount
   * @return  Builder instance
   */
  ...
  public void save(RecipeOutput consumer, Identifier id) {
    ...
    var advancement = this.buildOptionalAdvancement(key, "melting");
    // based on properties, choose which recipe to build
    if (oreRate != null) {
      consumer.accept(key, new OreMeltingRecipe(id, group, input, output, temperature, time, byproducts, oreRate, byproductRates), advancement);
    } else if (unitSizes != null) {
      consumer.accept(key, new DamageableMeltingRecipe(id, group, input, output, temperature, time, byproducts, unitSizes[0], List.of(Arrays.stream(unitSizes, 1, unitSizes.length).boxed().toArray(Integer[]::new))), advancement);
    } else {
      consumer.accept(key, new MeltingRecipe(id, group, input, output, temperature, time, byproducts), advancement);
    }
  }
}
