  ...
  public void handleByproducts(IMeltingContainer inv, IFluidHandler handler) {
    if (!byproducts.isEmpty()) {
      ...
      for (FluidOutput byproduct : byproducts) {
        handler.fill(byproduct.get().copyWithAmount(byproduct.getAmount() * cost), EXECUTE);
      }
    }
  }

  @Override
  public RecipeSerializer<? extends Recipe<IMeltingContainer>> getSerializer() {
    ...
    if (multiRecipes == null) {
      if (input.get().isHidden()) {
        ...
        multiRecipes = MaterialCastingLookup
          ...
          .filter(entry -> entry.getKey().canUseMaterial(inputId.getMaterialId()))
          .map(entry -> {
