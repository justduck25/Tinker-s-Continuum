...
public class MeltingRecipeLookup {
  ...
  public record MeltingFluid(Ingredient ingredient, FluidOutput result, int temperature) {
    ...
    public static final MeltingFluid EMPTY = new MeltingFluid(slimeknights.mantle.recipe.ingredient.EmptyIngredient.VANILLA, FluidOutput.EMPTY, 0);
    ...
    if (item != Items.AIR) {
      ...
      for (MeltingFluid fluid : FLUIDS) {
        try {
          ...
          // Some Mantle data/display ingredients such as ItemNameIngredient are not valid runtime predicates.
          // They are useful for datagen/recipe display, but mining speed predicates must not crash when probing meltability.
        }
      }
    }
