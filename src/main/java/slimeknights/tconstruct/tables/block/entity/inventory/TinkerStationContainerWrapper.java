...
public class TinkerStationContainerWrapper implements IMutableTinkerStationContainer {
  ...
  private MaterialRecipe findMaterialRecipe(ItemStack stack) {
    ...
    if (!world.isClientSide() && world.recipeAccess() instanceof RecipeManager manager) {
      Optional<MaterialRecipe> newRecipe = manager.getRecipeFor(TinkerRecipeTypes.MATERIAL.get(), inv, world).map(RecipeHolder::value);
