...
public class LazyResultContainer implements Container {
  ...
  /**
   * Sets the cached result from a server sync packet.
   * Used by client screens for custom recipes that are not available through the client recipe access in MC 26.1.
   * @param result  Synced result stack
   */
  public void setSyncedResult(ItemStack result) {
  ...
  /**
   * Updates the cached result from menu slot synchronization.
   * Vanilla sends initial menu contents through Slot#set(), so this must not be a no-op on the client.
   */
  @Override
