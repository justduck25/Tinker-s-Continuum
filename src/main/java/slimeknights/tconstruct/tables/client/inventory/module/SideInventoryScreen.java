...
public class SideInventoryScreen<P extends MultiModuleScreen<?>, C extends AbstractContainerMenu> extends ModuleScreen<P,C> {
  ...
  public SideInventoryScreen(P parent, C container, Inventory playerInventory, Component title, int slotCount, int columns, boolean rightSide, boolean connected) {
    ...
    this.moduleWidth = columns * this.slot.w + this.border.w * 2;
    this.moduleHeight = this.calcCappedYSize(this.slot.h * 10);
  ...
  protected int effectiveWidth() {
    return this.moduleWidth > 0 ? this.moduleWidth : super.effectiveWidth();
  }

  @Override
  protected int effectiveHeight() {
    return this.moduleHeight > 0 ? this.moduleHeight : super.effectiveHeight();
  }
  ...
  public boolean isSlotHovering(Slot slotIn, double mouseX, double mouseY) {
    return this.parent.isSlotHovering(slotIn, mouseX, mouseY) && this.shouldDrawSlot(slotIn);
  }
  ...
  public void updatePosition(int parentX, int parentY, int parentSizeX, int parentSizeY) {
    ...
    this.moduleHeight = this.calcCappedYSize(parentSizeY - 10);
    // slider needed?
    if (this.getDisplayedRows() < this.getTotalRows()) {
      this.slider.enable();
      this.moduleWidth = this.columns * this.slot.w + this.slider.width + 2 * this.border.w;
    }
    else {
      this.slider.disable();
      this.moduleWidth = this.columns * this.slot.w + this.border.w * 2;
    }
    ...
    this.border.setSize(this.moduleWidth, this.moduleHeight);

    int y = this.topPos + this.border.h;
    int h = this.moduleHeight - this.border.h * 2;
  ...
  public void extractBackground(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTicks) {
    ...
    int x = this.leftPos;
    int y = this.topPos;
    int midW = this.moduleWidth - this.border.w * 2;
  ...
  protected int drawSlots(GuiGraphicsExtractor graphics, int xPos, int yPos) {
    ...
    int height = this.moduleHeight - this.border.h * 2;
