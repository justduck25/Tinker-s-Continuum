...
public class PartBuilderScreen extends BaseTabbedScreen<PartBuilderBlockEntity,PartBuilderContainerMenu> {
  ...
  public PartBuilderScreen(PartBuilderContainerMenu container, Inventory playerInventory, Component title) {
    ...
    this.infoPanelScreen.setFullHeight(this.realHeight);
  ...
  public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
    ...
    if (this.infoPanelScreen.handleMouseClicked(event.x(), event.y(), event.button()) || this.tile == null) {
      return false;
    }
  ...
  public boolean mouseDragged(MouseButtonEvent event, double dx, double dy) {
    if (this.infoPanelScreen.handleMouseClickMove(event.x(), event.y(), event.button(), dx)) {
      return false;
    }
    ...
    if (this.clickedOnScrollBar && this.canScroll()) {
      ...
      this.sliderProgress = Mth.clamp(((float) event.y() - barStart - 7.5f) / (barEnd - barStart - SLIDER_TOP), 0, 1);
  ...
  public boolean mouseReleased(MouseButtonEvent event) {
    if (this.infoPanelScreen.handleMouseReleased(event.x(), event.y(), event.button())) {
      return false;
    }
