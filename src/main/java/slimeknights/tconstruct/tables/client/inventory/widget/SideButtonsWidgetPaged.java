...
public class SideButtonsWidgetPaged<T extends Button> extends SideButtonsWidget<T> {
    ...
    @Override
    public boolean handleMouseClicked(MouseButtonEvent event, boolean doubleClick) {
        if (event.button() == 0) {
            int button_num = this.buttons.size();
            if (button_num > columns * MAX_ROWS) { // Page button shows
                if (this.previousPageButton.mouseClicked(event, doubleClick)) {
                    this.clickedButtonIndex = 1;
                    return true;
                }
                if (this.nextPageButton.mouseClicked(event, doubleClick)) {
                    this.clickedButtonIndex = 2;
                    return true;
                }
            }
        }
        return super.handleMouseClicked(event, doubleClick);
    }
