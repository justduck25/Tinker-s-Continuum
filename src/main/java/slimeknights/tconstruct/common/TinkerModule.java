...
public abstract class TinkerModule {
  ...
  protected static final DeferredHolder<MapCodec<? extends ICondition>, MapCodec<ConfigEnabledCondition>> CONFIG_CONDITION = CONDITION_CODECS.register("config", () -> ConfigEnabledCondition.CODEC);
  protected static final DeferredHolder<MapCodec<? extends ICondition>, MapCodec<TagNotEmptyCondition<?>>> TAG_NOT_EMPTY_CONDITION = CONDITION_CODECS.register("tag_not_empty", () -> TagNotEmptyCondition.CODEC);
  protected static final DeferredHolder<MapCodec<? extends ICondition>, MapCodec<TagIntersectionPresentCondition<?>>> TAG_INTERSECTION_PRESENT_CONDITION = CONDITION_CODECS.register("tag_intersection_present", () -> TagIntersectionPresentCondition.CODEC);
  protected static final DeferredHolder<MapCodec<? extends ICondition>, MapCodec<TagDifferencePresentCondition<?>>> TAG_DIFFERENCE_PRESENT_CONDITION = CONDITION_CODECS.register("tag_difference_present", () -> TagDifferencePresentCondition.CODEC);
  // MC 1.21.6+ registers loot codecs directly; the old LootItemConditionType/LootItemFunctionType/LootPoolEntryType wrappers are gone.
  protected static final SynchronizedDeferredRegister<MapCodec<? extends net.minecraft.world.level.storage.loot.predicates.LootItemCondition>> LOOT_CONDITIONS = SynchronizedDeferredRegister.create(Registries.LOOT_CONDITION_TYPE, TConstruct.MOD_ID);
  protected static final DeferredHolder<MapCodec<? extends net.minecraft.world.level.storage.loot.predicates.LootItemCondition>, MapCodec<ConfigEnabledCondition>> LOOT_CONFIG_CONDITION = LOOT_CONDITIONS.register("config", () -> ConfigEnabledCondition.CODEC);
  protected static final DeferredHolder<MapCodec<? extends net.minecraft.world.level.storage.loot.predicates.LootItemCondition>, MapCodec<BlockOrEntityCondition>> BLOCK_OR_ENTITY_LOOT_CONDITION = LOOT_CONDITIONS.register("block_or_entity", () -> BlockOrEntityCondition.CODEC);
  protected static final DeferredHolder<MapCodec<? extends net.minecraft.world.level.storage.loot.predicates.LootItemCondition>, MapCodec<TagFilledLootCondition>> TAG_FILLED_LOOT_CONDITION = LOOT_CONDITIONS.register("tag_filled", () -> TagFilledLootCondition.CODEC);
  protected static final SynchronizedDeferredRegister<MapCodec<? extends net.minecraft.world.level.storage.loot.functions.LootItemFunction>> LOOT_FUNCTIONS = SynchronizedDeferredRegister.create(Registries.LOOT_FUNCTION_TYPE, TConstruct.MOD_ID);
  protected static final SynchronizedDeferredRegister<MapCodec<? extends net.minecraft.world.level.storage.loot.entries.LootPoolEntryContainer>> LOOT_ENTRIES = SynchronizedDeferredRegister.create(Registries.LOOT_POOL_ENTRY_TYPE, TConstruct.MOD_ID);

  // base item properties
  protected static final Item.Properties ITEM_PROPS = ItemDeferredRegister.registerSharedProperties(new Item.Properties());
  protected static final Item.Properties UNSTACKABLE_PROPS = ItemDeferredRegister.registerSharedProperties(new Item.Properties().stacksTo(1));
  ...
  /** Applies the current NeoForge registry ID to block properties created or reused inside a block supplier. */
  protected static BlockBehaviour.Properties blockProps(BlockBehaviour.Properties props) {
    return BlockDeferredRegister.setIdFromCurrentKey(props);
  }
  /** Creates item properties with the current NeoForge registry ID applied when inside an item supplier. */
  protected static Item.Properties itemProps() {
    return ItemDeferredRegister.setIdFromCurrentKey(new Item.Properties());
  }

  /** Creates unstackable item properties with the current NeoForge registry ID applied when inside an item supplier. */
  protected static Item.Properties unstackableProps() {
    return itemProps().stacksTo(1);
  }

  /** Called during construction to initialize the registers for this mod */
  public static void initRegisters() {
    IEventBus bus = TConstruct.MOD_EVENT_BUS;
    // gameplay singleton
    BLOCKS.register(bus);
    ITEMS.register(bus);
    FLUIDS.register(bus);
  ...
  protected static BlockBehaviour.Properties builder(SoundType soundType) {
    return BlockDeferredRegister.setIdFromCurrentKey(Block.Properties.of()).sound(soundType);
  }
  ...
  protected static BlockBehaviour.Properties glassBuilder(MapColor color) {
    return builder(color, SoundType.GLASS)
      ...
      .strength(0.3F).noOcclusion().isValidSpawn((state, getter, pos, type) -> false)
