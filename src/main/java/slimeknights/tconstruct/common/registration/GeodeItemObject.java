...
public class GeodeItemObject extends ItemObject<Item> {
  ...
  public GeodeItemObject(DeferredHolder<? extends Item, ? extends Item> shard, BlockDeferredRegister register, MapColor color, SoundType blockSound, SoundEvent chimeSound, Map<BudSize,SoundType> clusterSounds, int baseLight, Properties props) {
    super(shard);
    ...
    Function<Block, ? extends BlockItem> blockItem = block -> new BlockItem(block, ItemDeferredRegister.setIdFromCurrentKey(props));
    ToIntFunction<BlockState> crystalLight = light.apply(0);
    block = register.register(name + "_block", () -> new CrystalBlock(chimeSound, BlockDeferredRegister.setIdFromCurrentKey(BlockBehaviour.Properties.of()).mapColor(color).lightLevel(crystalLight).strength(1.5F).sound(blockSound).requiresCorrectToolForDrops()), blockItem);
    budding = register.register("budding_" + name, () -> new BuddingCrystalBlock(this, chimeSound, BlockDeferredRegister.setIdFromCurrentKey(BlockBehaviour.Properties.of()).mapColor(color).randomTicks().lightLevel(crystalLight).strength(1.5F).sound(blockSound).requiresCorrectToolForDrops().pushReaction(PushReaction.DESTROY)), blockItem);
    // buds
    Supplier<BlockBehaviour.Properties> budProps = () -> BlockDeferredRegister.setIdFromCurrentKey(BlockBehaviour.Properties.of()).mapColor(color).forceSolidOn().noOcclusion().randomTicks().strength(1.5F).requiresCorrectToolForDrops().pushReaction(PushReaction.DESTROY);
