...
public class AntigravityEffect extends TinkerEffect {
  ...
  public AntigravityEffect() {
    ...
    this.addAttributeModifier(Attributes.GRAVITY, slimeknights.tconstruct.TConstruct.getResource("antigravity"), -2, Operation.ADD_MULTIPLIED_TOTAL);
    ...
    if (living.getAttributeValue(Attributes.GRAVITY) < 0) {
      ...
      if (living.verticalCollision && !living.onGround()) {
        if (lastVelocity.twoTicks > 0.1f) {
          ...
          float height = (float)(lastVelocity.twoTicks * 10 - 3 - living.getAttributeValue(TinkerAttributes.SAFE_FALL_DISTANCE) - jumpLevel(living));
          if (height > 0.0F) {
      ...
      // handle friction
      float friction = 1f;
      if (living.verticalCollision && !living.onGround() && !living.shouldDiscardFriction()) {
        BlockPos above = BlockPos.containing(living.getX(), living.getBoundingBox().maxY + 0.1, living.getZ());
  ...
  private void onLivingJump(LivingJumpEvent event) {
    ...
    if (entity.hasEffect(TinkerEffects.holder(TinkerEffects.antigravity)) && entity.getAttributeValue(Attributes.GRAVITY) < 0) {
      Vec3 movement = entity.getDeltaMovement();
      entity.setDeltaMovement(movement.x, -movement.y, movement.z);
    }
  }

  private static int jumpLevel(LivingEntity living) {
    MobEffectInstance jump = living.getEffect(MobEffects.JUMP_BOOST);
  ...
  public boolean antigravityJump(Player player) {
    ...
    if (player.verticalCollision && !player.onGround() && !player.isInWater()
      && player.hasEffect(TinkerEffects.holder(TinkerEffects.antigravity)) && player.getAttributeValue(Attributes.GRAVITY) < 0 && !player.onClimbable()) {
