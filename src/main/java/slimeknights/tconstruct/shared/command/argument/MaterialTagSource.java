...
public record MaterialTagSource(MaterialManager manager) implements TagSource<IMaterial> {
  ...
  public List<Identifier> keysInTag(TagKey<IMaterial> tag) {
    ...
    return entries.stream().map(material -> Identifier.parse(material.getIdentifier().toString())).collect(Collectors.toList());
  }
  ...
  public Stream<Identifier> valueKeys() {
    return manager.getAllMaterials().stream().map(material -> Identifier.parse(material.getIdentifier().toString()));
  }
}
